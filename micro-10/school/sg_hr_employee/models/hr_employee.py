...
class hr_education_information(models.Model):
    ...
    comp_prog_knw = fields.Char('Computer Programs Knowledge', size=64, required=True)
...
class hr_employee(models.Model):
    ...
    def _check_employee_birthday(self):
        ...
        for rec in self:
            if rec.birthday:
                born = datetime.datetime.strptime(rec.birthday,
    ...
    def compute_joined_year(self):
        for emp in self:
            ...
            if emp.join_date:
                ...
                end_date = datetime.datetime.strptime(emp.join_date, DEFAULT_SERVER_DATE_FORMAT)
    ...
    def compute_pr_year(self):
        for emp in self:
            ...
            if emp.pr_date:
                start_date = datetime.datetime.today()
                end_date = datetime.datetime.strptime(emp.pr_date, DEFAULT_SERVER_DATE_FORMAT)
                difference_in_years = relativedelta(start_date, end_date).years
    ...
    def create(self, vals):
        ...
        if vals.get('job_id', '') or vals.get('emp_status', '') or vals.get('join_date', '') or vals.get('confirm_date', ''):
            self.env['employee.history'].create({
                                        ...
                                        'user_id': uid,
                                        'emp_status' : vals.get('emp_status', 'full_time'),
