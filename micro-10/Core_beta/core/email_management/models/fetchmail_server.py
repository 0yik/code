...
class vieterp_fetchmail_server(models.Model):
    ...
    def _fetch_mails(self):
        ...
        user_ids = self.env['res.users'].search([('incomming_mail_server','!=', False)])
        server_ids = [x.incomming_mail_server.id for x in user_ids]
        return self.search([('state', '=', 'done'), ('type', 'in', ['pop', 'imap']),('id', 'in', server_ids)]).fetch_mail()
    ...
    def fetch_mail(self):
        ...
        for server in self:
            if server.source_id and server.source_id.id:
                _logger.info('start checking for new emails on %s server %s', server.source_id.type, server.name)
                additionnal_context['fetchmail_server_id'] = server.id
                additionnal_context['fetchmail_server_inbox'] = True
                additionnal_context['server_type'] = server.source_id.type
                count, failed = 0, 0
                imap_server = None
                pop_server = None
                if server.source_id.type == 'imap':
                    try:
                        ...
                        for num in data[0].split():
                            ...
                            try:
                                ...
                                _logger.info('Failed to process mail from %s server %s.', server.source_id.type, server.name,
                                             exc_info=True)
                                failed += 1
                            ...
                            self._cr.commit()
                            count += 1
                        _logger.info("Fetched %d email(s) on %s server %s; %d succeeded, %d failed.", count, server.source_id.type,
                                     server.name, (count - failed), failed)
                    except Exception:
                        _logger.info("General failure when trying to fetch mail from %s server %s.", server.source_id.type,
                                     server.name, exc_info=True)
                    finally:
                        if imap_server:
                            imap_server.close()
                            imap_server.logout()
                elif server.source_id.type == 'pop':
                    try:
                        while True:
                            ...
                            _logger.info("Fetched %d email(s) on %s server %s; %d succeeded, %d failed.", num_messages,
                                         server.source_id.type, server.name, (num_messages - failed), failed)
                    except Exception:
                        _logger.info("General failure when trying to fetch mail from %s server %s.", server.source_id.type,
                                     server.name, exc_info=True)
                    finally:
