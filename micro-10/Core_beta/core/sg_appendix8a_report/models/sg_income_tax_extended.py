...
class hr_contract_income_tax(models.Model):
    ...
    def onchange_no_of_days(self):
        ...
        if self.from_date and self.to_date:
            start_date = datetime.strptime(self.from_date, DEFAULT_SERVER_DATE_FORMAT)
            end_date = datetime.strptime(self.to_date, DEFAULT_SERVER_DATE_FORMAT)
            if start_date > end_date:
                raise ValidationError(_("Please select valid date!"))
            diff = end_date - start_date
            noofday = str(diff.days)
            self.no_of_days =int(noofday) + 1
