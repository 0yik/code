...
class purchase_request(models.Model):
    ...
    product_ctg = fields.Many2one('pr.approving.matrix', string='Approval', track_visibility='onchange',required=False)
    ...
    def button_to_approve(self):
        ...
        if self.product_ctg:
	    approved_ids = self.product_ctg.line_ids.filtered(lambda r: r.check_amount(amount) == True)
	    self.write({'approved_ids':[(6,0,approved_ids.ids)]})
	    _logger.info("approved_ids ; "+str(approved_ids))
            #self.approved_ids = self.product_ctg.line_ids.filtered(lambda r: r.check_amount(amount) == True)
        approved_id = self.check_next_approver()
	_logger.info('approved_id : '+str(approved_id))
        if approved_id:
