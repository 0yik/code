...
class ReturnProductLine(models.TransientModel):
	_name = "stock.return.product.line"
	_rec_name = 'product_id'

	product_id = fields.Many2one('product.product', string="Product", required=True)
	quantity = fields.Float("Quantity", digits=dp.get_precision('Product Unit of Measure'), required=True)
	wizard_id = fields.Many2one('product.do.exchange', string="Wizard")
	move_id = fields.Many2one('stock.move', "Move")
	to_refund = fields.Boolean('To Refund')

class ReturnExchangeLine(models.TransientModel):
	_name = "stock.product.exchange.line"
	_rec_name = 'product_id'
...
class product_do_exchange(models.TransientModel):
	_name = 'product.do.exchange'

	product_exchange_moves = fields.One2many('stock.product.exchange.line', 'wizard_id', 'Moves')
	product_return_moves = fields.One2many('stock.return.product.line', 'wizard_id', 'Moves')
	...
	def default_get(self, fields):
		...
		res = super(product_do_exchange, self).default_get(fields)
		...
		if picking:
			...
			if 'product_return_moves' in fields:
				...
				res.update({'product_exchange_moves':product_return_moves})
	...
	@api.multi
	def create_pick_move(self, lines):
		picking = self.env['stock.picking'].browse(self.env.context['active_id'])
		picking_type_id = picking.picking_type_id.return_picking_type_id.id or picking.picking_type_id.id
		new_picking = picking.copy({
			'move_lines': [],
		...
		new_picking.write({'move_lines': create_line})
		if invoice_line:
	...
	def create_returns(self):
		...
		if self.product_exchange_moves:
			self.create_pick_move(self.product_exchange_moves)
