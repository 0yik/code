...
class QcInspectionLine(models.Model):
    ...
    def quality_test_check(self):
        for vals in self:
            if vals.question_type == 'qualitative':
                vals.success = vals.qualitative_value.ok
            else:
                # import pdb;pdb.set_trace()
                if vals.uom_id.id == vals.test_uom_category.id:
                    amount = vals.quantitative_value
                else:
                    amount = vals.uom_id._compute_quantity(
                        vals.quantitative_value,
                        vals.test_uom_category.id)
                vals.success = vals.max_value >= amount >= vals.min_value
