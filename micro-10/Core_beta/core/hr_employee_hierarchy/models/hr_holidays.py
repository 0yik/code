...
class hr_employee(models.Model):
    ...
    current_leave_state = fields.Selection(compute='_compute_leave_status', string="Current Leave Status",
        selection=[
            ...
            ('deputy_hod_approval', 'Waiting for Deputy HOD approval'),
...
class HrHolidays(models.Model):
    ...
    state = fields.Selection([('draft', 'New'), ('confirm', 'Waiting for SIC/Manager approval'),
                              ...
                              ('refuse', 'Refused'), ('validate1', 'Waiting Final Approval'),
                              ('validate', 'Approved'), ('cancel', 'Cancelled')],
        'State', readonly=True, default='draft', help='The state is set to \'Draft\', when a holiday request is created.\
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        admin_group = self.env.ref('sg_leave_types.group_admin_allocate_leave')
        hod_group = self.env.ref('hr_employee_hierarchy.group_leave_hod')
        if self._uid not in admin_group.users.ids and self._uid in hod_group.users.ids:
            emp_obj = self.env['hr.employee'].search([('user_id', '=', self._uid)], limit=1)
            if emp_obj and emp_obj.department_id:
                domain = [['department_id', '=', emp_obj.department_id.id]]
                args += domain
#         if self._uid not in admin_group.users.ids and not self._context.get('baseon_user'):
#             domain = [['user_id', '=', self._uid]]
#             args += domain
        if self._context.get('baseon_user') and self._uid not in admin_group.users.ids:
            sic_id = self.env.ref('hr_employee_hierarchy.group_leave_sic')
            assis_hod_id = self.env.ref('hr_employee_hierarchy.group_leave_tic')
            deputy_hod_id = self.env.ref('hr_employee_hierarchy.group_leave_deputy_hod')
            hod_id = self.env.ref('hr_employee_hierarchy.group_leave_hod')
            gm_id = self.env.ref('hr_employee_hierarchy.group_leave_gm')
            ed_id = self.env.ref('hr_employee_hierarchy.group_leave_ed')
            md_id = self.env.ref('hr_employee_hierarchy.group_leave_md')
            if self._uid in sic_id.users.ids:
                domain = [['state', 'in', ['confirm', 'validate']]]
                args += domain
            elif self._uid in assis_hod_id.users.ids:
                domain = [['state', 'in', ['tic_approval', 'validate']]]
                args += domain
            elif self._uid in deputy_hod_id.users.ids:
                domain = [['state', 'in', ['deputy_hod_approval', 'validate']]]
                args += domain
            elif self._uid in hod_id.users.ids:
                domain = [['state', 'in', ['hod_approval', 'validate']]]
                args += domain
            elif self._uid in gm_id.users.ids:
                domain = [['state', 'in', ['gm_approval', 'validate']]]
                args += domain
            elif self._uid in ed_id.users.ids:
                domain = [['state', 'in', ['ed_approval', 'validate']]]
                args += domain
            elif self._uid in md_id.users.ids:
                domain = [['state', 'in', ['md_approval', 'validate']]]
                args += domain
            else:
                domain = [['id', 'in', []]]
                args += domain
        res = super(HrHolidays, self).search(args, offset, limit, order, count=count)
        return res
...
#         ctx['approval_link'] = base_url + "/web#id="+ str(self.id) +"&view_type=form&model=hr.holidays"
        ...
        admin_group = self.env.ref('sg_leave_types.group_admin_allocate_leave')
        if self.is_recovery:
            template_id = self.env.ref('hr_employee_hierarchy.email_temp_leave_recovery_approval_manager')
...
#             template_id.subject = "Notification : Leave Recovery Application Review"
    ...
    def approve_next(self):
        ...
        if self.employee_id.user_id:
            if self.employee_id.user_id.id in hod_id.users.ids or self.employee_id.user_id.id in deputy_hod_id.users.ids:
                is_hod = True
                if self._uid in hod_id.users.ids:
                    raise ValidationError(_('Warning \n Only General Manager can approve, leave of Deputy HOD and HOD.'))
            if self.employee_id.user_id.id in gm_id.users.ids:
                is_gm = True
                if self._uid not in md_id.users.ids:
                    raise ValidationError(_('Warning \n Only Managing Director can approve, leave of General Manager.'))
            if self.employee_id.user_id.id in ed_id.users.ids:
                is_ed = True
                if self._uid not in md_id.users.ids:
                    raise ValidationError(_('Warning \n Only Managing Director can approve, leave of Executive Manager.'))

        if self._uid not in user_ids and not is_hod and not is_gm and not is_ed and self._uid not in admin_group.users.ids:
        ...
        if self._uid not in hod_id.users.ids and not is_hod and not is_gm and not is_ed and self._uid not in admin_group.users.ids:
...
#         if self._uid != self.next_manager_id.user_id.id:
        if self._uid not in gm_id.users.ids and is_hod and self._uid not in admin_group.users.ids:
            raise ValidationError(_('Warning \n Only GM of Department can Approve this Leave.'))
        if self._uid not in md_id.users.ids and (is_ed or is_gm) and self._uid not in admin_group.users.ids:
            raise ValidationError(_('Warning \n Only MD of Department can Approve this Leave.'))
        ...
        job_id = self.env['hr.job'].search([('name', '=', 'HR HOD')], limit=1, order="id desc")
        if job_id:
            hr_hod = self.env['hr.employee'].search([('job_id', '=', job_id.id)])
        hr_hod_mails = ''
        for obj in hr_hod:
        ...
        template_id.email_from = mail_from
        template_id.email_to = mail_to + hr_hod_mails
        ctx = self.env.context.copy() if self.env.context else {}
#         if ctx.get('default_state') == 'next_approval':
        if ctx.get('default_state'):
            ctx.pop('default_state')
...
#         template_id.send_mail(self.id, force_send=True)
    ...
    def check_skip(self, manager_ids):
        ...
        for user in manager_ids.users:
            ...
            if manager_ids in [gm_id, ed_id, md_id]:
                ...
                emp_id = self.env['hr.employee'].sudo().search([('user_id', '=', user.id), ('department_id', '=', self.employee_id.department_id.id)], limit=1)
                if not emp_id and self.employee_id.department_id.parent_id:
                    emp_id = self.env['hr.employee'].sudo().search([('user_id', '=', user.id), ('department_id', '=', self.employee_id.department_id.parent_id.id)], limit=1)
    ...
    def leave_approve_hierarchy(self, from_create=False):
        ...
        if self.state == 'draft' or self.state in ['pm_approval', 'sup_approval', 'eng_approval']:
            ...
            if assis_hod_id.id in group_ids and (self.employee_id.user_id.id not in assis_hod_id.users.ids and self.employee_id.user_id.id not in deputy_hod_id.users.ids and self.employee_id.user_id.id not in hod_id.users.ids and self.employee_id.user_id.id not in gm_id.users.ids and self.employee_id.user_id.id not in ed_id.users.ids and self.employee_id.user_id.id not in md_id.users.ids):
                ...
                if not skip_approve:
                    ...
                    return self.state
            if deputy_hod_id.id in group_ids and (self.employee_id.user_id.id not in deputy_hod_id.users.ids and self.employee_id.user_id.id not in hod_id.users.ids and self.employee_id.user_id.id not in gm_id.users.ids and self.employee_id.user_id.id not in ed_id.users.ids and self.employee_id.user_id.id not in md_id.users.ids):
                skip_approve = self.check_skip(deputy_hod_id)
                if not skip_approve:
                    self.state = 'deputy_hod_approval'
                    return self.state
        ...
        if self.state == 'tic_approval':
            if deputy_hod_id.id in group_ids:
                skip_approve = self.check_skip(deputy_hod_id)
                if not skip_approve:
                    self.state = 'deputy_hod_approval'
                    return self.state
            ...
            if md_id.id in group_ids:
                ...
                if not skip_approve:
                    self.state = 'md_approval'
                    return self.state
        if self.state == 'deputy_hod_approval':
            if hod_id.id in group_ids:
                skip_approve = self.check_skip(hod_id)
                if not skip_approve:
                    self.state = 'hod_approval'
...
#                 if not skip_approve:
    ...
    def approve_sic(self):
        ...
        if self.employee_id.hierarchy_id:
            dept_ids = []
            dept_ids.append(self.employee_id.department_id.id)
            if self.employee_id.department_id.parent_id:
                dept_ids.append(self.employee_id.department_id.parent_id.id)
            employee_ids = self.env['hr.employee'].sudo().search([('department_id', 'in', dept_ids)])
            user_ids = [x.user_id.id for x in employee_ids]
            if self._uid not in user_ids and self._uid not in admin_group.users.ids:
            ...
            if state =='deputy_hod_approval':
                self.notify_email_manager(deputy_hod_id)
    ...
    def approve_deputy(self):
        ...
        if self.employee_id.hierarchy_id:
            ...
            user_ids = [x.user_id.id for x in employee_ids]
            if self._uid not in user_ids and self._uid not in admin_group.users.ids:
                raise ValidationError(_('Warning \n Only Deputy of %s Department can Approve this Leave.') % (self.employee_id.department_id.name))
...
#                     raise ValidationError(_('Warning \n Only ED of %s Department can Approve this Leave.') % (self.employee_id.department_id.name))
    ...
    def action_refuse(self):
        admin_group = self.env.ref('sg_leave_types.group_admin_allocate_leave')
        if self.state !=False:
            if self._uid == self.employee_id.user_id.id and self._uid not in admin_group.users.ids:
            ...
            if (self._uid in engineer_grp.users.ids or self._uid in supervisor_grp.users.ids or self._uid in worker_grp.users.ids) and self._uid not in admin_group.users.ids:
                flag = True

            if self._uid in site_mgr_id.users.ids and self.state == 'confirm' and self._uid not in admin_group.users.ids and (self.employee_id.user_id.id in site_mgr_id.users.ids or self.employee_id.user_id.id in assis_hod_id.users.ids or self.employee_id.user_id.id in hod_id.users.ids or self.employee_id.user_id.id in gm_id.users.ids or self.employee_id.user_id.id in ed_id.users.ids):
                flag = True
            if self.state == 'tic_approval' and (self.employee_id.user_id.id in assis_hod_id.users.ids or self.employee_id.user_id.id in hod_id.users.ids or self.employee_id.user_id.id in gm_id.users.ids or self.employee_id.user_id.id in ed_id.users.ids) and self._uid not in admin_group.users.ids:
                if self._uid in assis_hod_id.users.ids or self._uid in site_mgr_id.users.ids:
                    flag = True
            if self.state == 'deputy_hod_approval' and (self.employee_id.user_id.id in assis_hod_id.users.ids or self.employee_id.user_id.id in deputy_hod_id.users.ids or self.employee_id.user_id.id in hod_id.users.ids or self.employee_id.user_id.id in gm_id.users.ids or self.employee_id.user_id.id in ed_id.users.ids) and self._uid not in admin_group.users.ids:
                if self._uid in assis_hod_id.users.ids or self._uid in site_mgr_id.users.ids or self._uid in assis_hod_id.users.ids:
                    flag = True
            if self.state == 'hod_approval' and (self.employee_id.user_id.id in hod_id.users.ids or self.employee_id.user_id.id in gm_id.users.ids or self.employee_id.user_id.id in ed_id.users.ids) and self._uid not in admin_group.users.ids:
                if self._uid in assis_hod_id.users.ids or self._uid in site_mgr_id.users.ids or self._uid in assis_hod_id.users.ids or self._uid in hod_id.users.ids:
                    flag = True

            if (self._uid not in gm_id.users.ids or self._uid not in ed_id.users.ids) and self._uid not in admin_group.users.ids:
...
#                             break
                    ...
                    for project_id in project_ids:
                        if self._uid in project_id.user_id.ids:
                            tmp = True
                            break
                dept_ids = [self.employee_id.department_id]
                if self.employee_id.department_id.parent_id:
                    dept_ids.append(self.employee_id.department_id.parent_id)
                if emp and emp.department_id not in dept_ids and not tmp:
...
#         NOTE : set status
        ...
        if res.holiday_status_id.name == 'OIL' and res.type == 'add':
            deputy_above_ids = deputy_hod_id.users.ids + hod_id.users.ids + gm_id.users.ids + ed_id.users.ids + md_id.users.ids
            if res.employee_id.user_id.id in md_id.users.ids:
            ...
            if not s1.intersection(s2) and res.employee_id.user_id.id not in deputy_above_ids:
...
#                         template_id.subject = "Notification : Leave Recovery Application Review"
        ...
        if not res.employee_id.hierarchy_id:
            if res.employee_id.user_id.id in site_mgr_id.users.ids:
                ...
                res.state = 'deputy_hod_approval'
            elif res.employee_id.user_id.id in deputy_hod_id.users.ids:
            ...
            if res.employee_id.hierarchy_id:
                ...
                if state =='deputy_hod_approval':
                    res.notify_email_manager(deputy_hod_id)
                if state =='hod_approval':
                    res.notify_email_manager(hod_id)
                if state =='gm_approval':
...
#         NOTE : set status
        ...
        if self.holiday_status_id.name == 'OIL' and self.type == 'add':
            ...
            s1 = set(user_ids)
            s2 = set(hod_id.users.ids)
            if not s1.intersection(s2) and self.employee_id.user_id.id not in deputy_above_ids:
                raise UserError(_('Your department does not have HOD. Off in Lieu is approved by HOD.'))
...
#                         template_id.subject = "Notification : Leave Recovery Application Review"
        ...
        if not self.employee_id.hierarchy_id:
            if self.employee_id.user_id.id in site_mgr_id.users.ids:
                ...
                self.state = 'deputy_hod_approval'
            elif self.employee_id.user_id.id in deputy_hod_id.users.ids:
                self.state = 'hod_approval'
