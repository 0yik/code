...
class PtkpCategory(models.Model):
    ...
    tarif_ptkp=fields.Float(string="Tarif PTKP")
...
class Ptkp(models.Model):
    ...
    def create(self, values):
        res=super(Ptkp, self).create(values)
        if 'date_start' in values:
            ...
            for line in tarif:
                ...
                if d1 and d2:
                    ...
                    days=abs((d2 - d1).days)
                    diff[line.id]=days
            val=min(diff, key=diff.get)
            obj=self.search([('id','=',val)])
            for line in obj.line_ids:
                obj1=self.env['l10n_id.ptkp_category'].search([('id','=',line.ptkp_category_id.id)])
                v={'tarif_ptkp':line.ptkp_rate}
                obj1.write(v)
        return res
