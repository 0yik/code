...
class hr_holiday_status(models.Model):
    ...
    @api.onchange('cut_off_date')
    def onchange_cut_off_date(self):
        warning = {}
        if self.cut_off_date and (self.cut_off_date > 31):
            self.cut_off_date = False
            warning = {'title': 'Value Error', 'message': "Cut off Date should be less than 31"}
