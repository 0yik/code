...
class StockPicking(models.Model):
    ...
    def write(self, vals):
        ...
        for rec in self:
            if rec.booking_order_id:
                if rec.state == 'done':
                    for line in rec.move_lines:
                        if rec.booking_order_line_id.state in ['pending','out']:
                            if rec.picking_type_id.id == outgoing_type_id.id:
                                line.booking_order_line_id.state = 'out'
                                line.booking_order_line_id.order_id.state = 'out'
                            if rec.picking_type_id.id == incoming_type_id.id:
