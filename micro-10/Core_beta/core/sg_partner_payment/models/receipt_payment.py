        ...
        for record in self:
            ...
            if record.diff_amount:
                if self.type == 'customer':
                    ...
                    payment_method_id = self.env['ir.model.data'].xmlid_to_res_id('account.account_payment_method_manual_out')
                    payment_type = 'outbound'
                payment = record.create_payments(False, payment_method_id, payment_type, record.diff_amount)
                payment.difference = record.diff_amount
                payment.total_residual = record.diff_amount
                if record.payment_difference_handling == 'reconcile':
                    payment.payment_difference_handling = 'reconcile'
                    payment.writeoff_account_id = record.writeoff_acc_id.id
                payment.post()
                record.write({'payment_ids': [(4, payment.id)]})
