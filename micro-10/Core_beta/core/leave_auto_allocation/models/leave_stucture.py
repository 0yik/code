        ...
        for leaves_structure in leaves_structure_ids:
            for config in leaves_structure.holiday_group_config_line_ids:
                if config.by_joined_year or config.sick_leave or config.hospitalisation_leave:
                    ...
                    employees_ids = self.env['hr.employee'].search([('leave_config_id','=',config.holiday_group_config_id.id),('id','=',43)])
                    for employee in employees_ids:
                        ...
                        if config.by_joined_year:
                            ...
                            lines.sort()
                            for line in lines:
                                line_obj = joined_year_obj.search([('year','=',line)])
                                if employee.joined_year >= line:
                                    final_year = line_obj[0].days
                                    continue
                        ...
                        if config.hospitalisation_leave:
                            sick_leaves = holiday_obj.search([('employee_id','=',employee.id), ('hr_year_id','=',year), ('holiday_status_id.name', '=', 'SL'),('type', '=', 'remove'), ('state', '=', 'validate')])
                            sick_leave_used_count = sum([leave.number_of_days_temp for leave in sick_leaves])
                            vals['number_of_days_temp'] = final_year - sick_leave_used_count - leave_used_count
...
class MaternityLeave(models.Model):
    ...
    days = fields.Integer('Default Leave Weeks')
    config_line_id = fields.Many2one('holiday.group.config.line', string="Line")
...
class ExtendedChildcareLeave(models.Model):
    ...
    nationality_ids = fields.Many2many('res.country', 'res_country_ext_childcare_leave', 'ext_childcare_id', 'res_country_id', string="Child's Nationality")
    days = fields.Integer('Default Leave Days')
    config_line_id = fields.Many2one('holiday.group.config.line', string="Config")
...
class SharedParentalLeave(models.Model):
    ...
    relationship = fields.Selection([('son','Son'),('daughter','Daughter')], string="Relationship")
    nationality_ids = fields.Many2many('res.country', 'res_country_shared_parental_leave', 'parental_id', 'res_country_id', string="Child's Nationality")
    days = fields.Integer('Default Leave Weeks')
    config_line_id = fields.Many2one('holiday.group.config.line', string="Line")
    birth_date = fields.Date("Child's Birth Date")
