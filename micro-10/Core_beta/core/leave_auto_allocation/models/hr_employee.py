...
class Dependets(models.Model):
    ...
    def create(self, vals):
        dependent = super(Dependets, self).create(vals)
        if dependent.relation_ship in ('son','daughter'):
            self.allocate_default_leaves(dependent)
        return dependent

    @api.multi
    def unlink(self):
        for dependent in self:
            dependents = self.search([('employee_id','=',dependent.employee_id.id), ('id','!=',dependent.id),('relation_ship','in',('son','daughter'))])
            if not dependents:
                leave = self.env['hr.holidays'].search([('maternity_leave','=', True), ('employee_id','=',dependent.employee_id.id),('type','=','add')])
                if leave:
                    leave.with_context({'allow_delete':True}).unlink()
        return super(Dependets, self).unlink()
...
#           Check for the Maternity Leave
    ...
    def create_holiday(self, config, dependent, line, leave_type=None):
        ...
        if leave_type in ['maternity_leave','paternity_leave','shared_parental_leave'] :
            vals['maternity_leave'] = True
            contract_ids = contract_obj.search([('employee_id', '=', dependent.employee_id.id)])
            if contract_ids:
                weekdays = len(contract_obj.browse(max(contract_ids.ids)).working_hours.get_weekdays())
                vals['number_of_days_temp'] = (weekdays * line[0].days) - leave_used_count
