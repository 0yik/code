...
var account_report_generic = Widget.extend(ControlPanelMixin, {
    ...
    get_html: function() {
        ...
        return new Model('account.report.context.common').call('return_context', [self.report_model, self.given_context, self.report_id]).then(function (result) {
            ...
            return self.context_model.call('get_html_and_data', [self.context_id, self.given_context], {context: session.user_context}).then(function (result) {
                ...
                if (result.available_partners) {
                    self.report_context.available_partners = result.available_partners;
                }
                if (result.available_currency) {
                    self.report_context.available_currency = result.available_currency;
                }
                self.render_buttons();
    ...
    render_buttons: function() {
        ...
        this.$buttons.siblings('.o_account-widget-pdf').bind('click', function () {
            var aging_filter_cmp = (self.report_context.aging_filter_cmp || false).toString();
            var aging_due_filter_cmp = (self.report_context.aging_due_filter_cmp || false).toString();
            var filter_local_currency = (self.report_context.filter_local_currency || false).toString();
            var filter_original_currency = (self.report_context.filter_original_currency || false).toString();
            var url = self.controller_url.replace('output_format', 'pdf').concat('/',aging_filter_cmp,'/',aging_due_filter_cmp,'/',filter_local_currency,'/',filter_original_currency);
            framework.blockUI();
            session.get_file({
                url: url,
        ...
        var aging_filter_cmp = (self.report_context.aging_filter_cmp || false).toString();
            ...
            var filter_local_currency = (self.report_context.filter_local_currency || false).toString();
            var filter_original_currency = (self.report_context.filter_original_currency || false).toString();
            var url = self.controller_url.replace('output_format', 'xlsx').concat('/',aging_filter_cmp,'/',aging_due_filter_cmp,'/',filter_local_currency,'/',filter_original_currency);
            framework.blockUI();
            session.get_file({
                url: url,
                complete: framework.unblockUI,
        ...
        this.$buttons.siblings('.o_account-widget-xml').bind('click', function () {
            ...
            return new Model('account.financial.html.report.xml.export').call('check', [self.report_model, self.report_id]).then(function (check) {
                ...
                var filter_local_currency = (self.report_context.filter_local_currency || false).toString();
                var filter_original_currency = (self.report_context.filter_original_currency || false).toString();
                var url = self.controller_url.replace('output_format', 'xml').concat('/',aging_filter_cmp,'/',aging_due_filter_cmp,'/',filter_local_currency,'/',filter_original_currency);
                    framework.blockUI();
                    session.get_file({
                        url: url,
                        complete: framework.unblockUI,
