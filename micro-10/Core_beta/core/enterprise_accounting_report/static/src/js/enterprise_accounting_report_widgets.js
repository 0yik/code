odoo.define('enterprise_accounting_report.ReportWidget', function (require) {
'use strict';
...
var ReportWidget = Widget.extend({
    events: {
        ...
        'click .o_enterprise_accounting_report_foldable': 'fold',
        'click .o_enterprise_accounting_report_unfoldable': 'unfold',
        'click .fa-trash-o': 'rmContent',
        'click .o_enterprise_accounting_report_saved_summary > span': 'editSummary',
        'click .o_enterprise_accounting_report_edit_summary_pencil': 'editSummary',
        "click input[name='summary']": 'onClickSummary',
        "click button.saveSummary": 'saveSummary',
        'click button.saveContent': 'saveContent',
        'click .o_enterprise_accounting_report_add-footnote': 'footnoteFromDropdown',
        'click .o_enterprise_accounting_report_web_action': 'outboundLink',
        'click .o_enterprise_accounting_report_footnote_sup': 'goToFootNote',
    },
    ...
    start: function() {
        QWeb.add_template("/enterprise_accounting_report/static/src/xml/account_report_financial_line.xml");
    ...
    onKeyPress: function(e) {
        if ((e.which === 70) && (e.ctrlKey || e.metaKey) && e.shiftKey) { // Fold all
            this.$(".o_enterprise_accounting_report_foldable").trigger('click');
        }
        else if ((e.which === 229) && (e.ctrlKey || e.metaKey) && e.shiftKey) { // Unfold all
            this.$(".o_enterprise_accounting_report_unfoldable").trigger('click');
        }
    },
    // Changes the placeholder into an editable textarea and give it the focus
    onClickSummary: function(e) {
        e.stopPropagation();
        $(e.target).parents("div.o_enterprise_accounting_report_summary").html(QWeb.render("editSummary"));
    ...
    saveSummary: function(e) {
        e.stopPropagation();
        e.preventDefault();
        var context_id = $(e.target).parents("div.o_enterprise_accounting_report_page").data("context");
        var summary = $(e.target).siblings("textarea[name='summary']").val().replace(/\r?\n/g, '<br />').replace(/\s+/g, ' ');
        if (summary) // If it isn't empty, display it normally
            $(e.target).parents("div.o_enterprise_accounting_report_summary").html(QWeb.render("savedSummary", {summary : summary}));
        else // If it's empty, delete the summary and display the default placeholder
            $(e.target).parents("div.o_enterprise_accounting_report_summary").html(QWeb.render("addSummary"));
    ...
    footnoteFromDropdown: function(e) {
        ...
        e.preventDefault();
        var self = this;
        var context_id = $(e.target).parents("div.o_enterprise_accounting_report_body").find('div.o_enterprise_accounting_report_page').data("context");
        ...
        if(curFootNoteTarget.parents('div.dropdown').find('sup').length === 0) { // Make sure there's no footnote yet
            ...
            var save = function () {
                var note = $content.find(".o_enterprise_accounting_report_footnote_note").val().replace(/\r?\n/g, '<br />').replace(/\s+/g, ' '); // Get the note and strip off extra spaces and line returns
                return  self.context_model.call('get_next_footnote_number', [[parseInt(context_id, 10)]]).then(function (footNoteSeqNum) { // Get the next sequence number
                    ...
                    .filter([['id', '=', context_id]]).first().then(function (context) {
                        new Model('account.report.footnotes.manager').call('add_footnote', [[parseInt(context.footnotes_manager_id[0], 10)], $content.find(".o_enterprise_accounting_report_footnote_type").val(), $content.find(".o_enterprise_accounting_report_footnote_target_id").val(), $content.find(".o_enterprise_accounting_report_footnote_column").val(), footNoteSeqNum, note]);
                        self.$("div.o_enterprise_accounting_report_page").append(QWeb.render("savedFootNote", {num: footNoteSeqNum, note: note})); // Render the footnote at the bottom
                    });
                });
            }
    ...
    editSummary: function(e) {
        e.stopPropagation();
        e.preventDefault();
        var $el = $(e.target).parents('.o_enterprise_accounting_report_summary').find('.o_enterprise_accounting_report_saved_summary span');
        ...
        var par = $el.parents("div.o_enterprise_accounting_report_summary");
        $el.parents("div.o_enterprise_accounting_report_summary").html(QWeb.render("editSummary", {summary: text})); // Render the textarea
    ...
    clickPencil: function(e) {
        ...
        if ($(e.target).parents("p.footnote").length > 0) { // If it's to edit a footnote at the bottom
            $(e.target).parents('.footnote').attr('class', 'o_enterprise_accounting_report_footnote_edit');
            var $el = $(e.target).parents('.o_enterprise_accounting_report_footnote_edit').find('span.text');
            var text = $el.html().replace(/\s+/g, ' ').replace(/\r?\n/g, '').replace(/<br>/g, '\n').replace(/(\n\s*)+$/g, ''); // Remove unnecessary spaces and line returns
    ...
    saveContent: function(e) {
        ...
        var footNoteSeqNum = $(e.target).parents('p.o_enterprise_accounting_report_footnote_edit').text().split('.')[0];
        if ($(e.target).parents("p.o_enterprise_accounting_report_footnote_edit").length > 0) {
            $(e.target).parents("p.o_enterprise_accounting_report_footnote_edit").attr('class', 'footnote'); // Remove textarea and change back class name
    ...
    rmContent: function(e) {
        ...
        var context_id = window.$("div.o_enterprise_accounting_report_page").data("context");
    ...
    fold: function(e) {
        ...
        for (el in $nextEls) { // While domain lines are found, keep hiding them. Stop when they aren't domain lines anymore
            $el = $($nextEls[el]).find("td span.o_enterprise_accounting_report_domain_line_1, td span.o_enterprise_accounting_report_domain_line_2, td span.o_enterprise_accounting_report_domain_line_3");
            if ($el.length === 0)
                break;
            else {
        ...
        var active_id = $(e.target).parents('tr').find('td.o_enterprise_accounting_report_foldable').data('id');
        $(e.target).parents('tr').toggleClass('o_enterprise_accounting_report_unfolded'); // Change the class, rendering, and remove line from model
        $(e.target).parents('tr').find('td.o_enterprise_accounting_report_foldable').attr('class', 'o_enterprise_accounting_report_unfoldable ' + active_id);
        $(e.target).parents('tr').find('span.o_enterprise_accounting_report_foldable').replaceWith(QWeb.render("unfoldable", {lineId: active_id}));
    ...
    unfold: function(e) {
        ...
        e.preventDefault();
        var self = this;
        var report_name = window.$("div.o_enterprise_accounting_report_page").data("report-name");
        var context_id = window.$("div.o_enterprise_accounting_report_page").data("context");
        var active_id = $(e.target).parents('tr').find('td.o_enterprise_accounting_report_unfoldable').data('id');
        return this.context_model.call('add_line', [[parseInt(context_id, 10)], parseInt(active_id, 10)]).then(function () { // First add the line to the model
                    ...
                    var f = function (lines) {// After loading the line
                        return self.context_model.call('get_footnotes_from_lines', [[parseInt(context_id, 10)], lines]).then(function (footnotes) {
                            ...
                            for (footnote in footnotes) {
                                self.$("div.o_enterprise_accounting_report_page").append(QWeb.render("savedFootNote", {num: footnotes[footnote].number, note: footnotes[footnote].text}));
                            }
            ...
            $(e.target).parents('tr').toggleClass('o_enterprise_accounting_report_unfolded'); // Change the class, and rendering of the unfolded line
            $(e.target).parents('tr').find('td.o_enterprise_accounting_report_unfoldable').attr('class', 'o_enterprise_accounting_report_foldable ' + active_id);
            $(e.target).parents('tr').find('span.o_enterprise_accounting_report_unfoldable').replaceWith(QWeb.render("foldable", {lineId: active_id}));
        });
    },
