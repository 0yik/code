...
class report_account_aged_partner(models.AbstractModel):
    ...
    def _lines(self, context, line_id=None):
            ...
            if partner_id in partner_ids:
                for line in amls.get(partner_id):
                    ...
                    if partner_id not in partner_total_amount:
                        partner_total_amount.update({partner_id: round(invoice_sign * abs(aml.amount_currency or aml.debit or aml.credit), 2)})
                    else:
                        partner_total_amount.update({partner_id: partner_total_amount.get(partner_id) + round(invoice_sign * abs(aml.amount_currency or aml.debit or aml.credit), 2)})
                    # Partner Due Total amount
                    if self.env.context.get('filter_original_currency'):
                        if partner_id not in partner_due_amount:
                            partner_due_amount.update({partner_id: round(abs(aml.amount_residual_currency or aml.amount_residual), 2)})
                        else:
                            partner_due_amount.update({partner_id: partner_due_amount.get(partner_id) + round(abs(aml.amount_residual_currency or aml.amount_residual), 2)})
                    else:
                        if partner_id not in partner_due_amount:
                            partner_due_amount.update({partner_id: round(abs(aml.amount_residual), 2)})
                        else:
                            partner_due_amount.update({partner_id: partner_due_amount.get(partner_id) + round(abs(aml.amount_residual), 2)})

                # Adding receipts and payments total
                domain = [('partner_id','=',partner_id),('payment_date','<=',self.env.context.get('date_to')),('state','=','posted')]
                payment_ids = self.env['account.payment'].search(domain)
                for payment in payment_ids:
                    move_line_id = payment.move_line_ids.filtered(lambda r: r.account_id.reconcile)
                    if move_line_id.reconciled:
                        continue
                    # Currency Total
                    if partner_id not in partner_total_amount:
                        partner_total_amount.update({partner_id: round(abs(move_line_id.amount_currency or move_line_id.debit or move_line_id.credit), 2)})
                    else:
                        partner_total_amount.update({partner_id: partner_total_amount.get(partner_id) + round(abs(move_line_id.amount_currency or move_line_id.debit or move_line_id.credit), 2)})
                    # Residual Total
                    if self.env.context.get('filter_original_currency'):
                        if partner_id not in partner_due_amount:
                            partner_due_amount.update({partner_id: round(abs(move_line_id.amount_residual_currency or move_line_id.amount_residual), 2)})
                        else:
                            partner_due_amount.update({partner_id: partner_due_amount.get(partner_id) + round(abs(move_line_id.amount_residual_currency or move_line_id.amount_residual), 2)})
                    else:
                        if partner_id not in partner_due_amount:
                            partner_due_amount.update({partner_id: round(abs(move_line_id.amount_residual), 2)})
                        else:
                            partner_due_amount.update({partner_id: partner_due_amount.get(partner_id) + round(abs(move_line_id.amount_residual), 2)})
        ...
        for values in results:
            ...
            for line in amls[values['partner_id']]:
                ...
                if age.days >= 0 and age.days <= 30:
                    ...
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({0: amount + (invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual))})
                        partner_totals_final.update({0: partner_totals_final.get(0) + (invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual))})
                    else:
                        partner_totals.get(values['partner_id']).update({0: amount + (invoice_sign * abs(aml.amount_residual))})
                ...
                if age.days >= 31 and age.days <= 60:
                    ...
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({1: amount + (invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual))})
                        partner_totals_final.update({1: partner_totals_final.get(1) + (invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual))})
                    else:
                        partner_totals.get(values['partner_id']).update({1: amount + (invoice_sign * abs(aml.amount_residual))})
                        partner_totals_final.update({1: partner_totals_final.get(1) + (invoice_sign * abs(aml.amount_residual))})
                # 60-90 days
                if age.days >= 61 and age.days <= 90:
                    amount = partner_totals.get(values['partner_id']).get(2)
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({2: amount + (invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual))})
                        partner_totals_final.update({2: partner_totals_final.get(2) + (invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual))})
                    else:
                ...
                if age.days >= 91 and age.days <= 120:
                    ...
                    if self.env.context.get('filter_original_currency'):
                        ...
                        partner_totals_final.update({3: partner_totals_final.get(3) + (invoice_sign * abs(aml.amount_residual))})
                # >120 days
                if age.days > 120:
                    amount = partner_totals.get(values['partner_id']).get(4)
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({4: amount + (invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual))})
                        partner_totals_final.update({4: partner_totals_final.get(4) + (invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual))})
                    else:
            ...
            domain = [('partner_id','=',values['partner_id']),('payment_date','<=',self.env.context.get('date_to')),('state','=','posted')]
            payment_ids = self.env['account.payment'].search(domain)
            for payment in payment_ids:
                move_line_id = payment.move_line_ids.filtered(lambda r: r.account_id.reconcile)
                if move_line_id.reconciled:
                ...
                if age.days >= 0 and age.days <= 30:
                    amount = partner_totals.get(values['partner_id']).get(0)
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({0: amount + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                        partner_totals_final.update({0: partner_totals_final.get(0) + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                    else:
                        partner_totals.get(values['partner_id']).update({0: amount + abs(move_line_id.amount_residual)})
                        partner_totals_final.update({0: partner_totals_final.get(0) + abs(move_line_id.amount_residual)})
                # 30-60 days
                if age.days >= 31 and age.days <= 60:
                    amount = partner_totals.get(values['partner_id']).get(1)
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({1: amount + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                        partner_totals_final.update({1: partner_totals_final.get(1) + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                    else:
                        partner_totals.get(values['partner_id']).update({1: amount + abs(move_line_id.amount_residual)})
                        partner_totals_final.update({1: partner_totals_final.get(1) + abs(move_line_id.amount_residual)})
                # 60-90 days
                if age.days >= 61 and age.days <= 90:
                    amount = partner_totals.get(values['partner_id']).get(2)
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({2: amount + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                        partner_totals_final.update({2: partner_totals_final.get(2) + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                    else:
                        partner_totals.get(values['partner_id']).update({2: amount + abs(move_line_id.amount_residual)})
                        partner_totals_final.update({2: partner_totals_final.get(2) + abs(move_line_id.amount_residual)})
                # 90-120 days
                if age.days >= 91 and age.days <= 120:
                    amount = partner_totals.get(values['partner_id']).get(3)
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({3: amount + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                        partner_totals_final.update({3: partner_totals_final.get(3) + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                    else:
                        partner_totals.get(values['partner_id']).update({3: amount + abs(move_line_id.amount_residual)})
                        partner_totals_final.update({3: partner_totals_final.get(3) + abs(move_line_id.amount_residual)})
                # >120 days
                if age.days > 120:
                    amount = partner_totals.get(values['partner_id']).get(4)
                    if self.env.context.get('filter_original_currency'):
                        partner_totals.get(values['partner_id']).update({4: amount + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                        partner_totals_final.update({4: partner_totals_final.get(4) + abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)})
                    else:
                        partner_totals.get(values['partner_id']).update({4: amount + abs(move_line_id.amount_residual)})
                        partner_totals_final.update({4: partner_totals_final.get(4) + abs(move_line_id.amount_residual)})
            ...
            if values['partner_id'] in context.unfolded_partners.ids:
                for line in amls[values['partner_id']]:
                        ...
                        if invoice.date_due:
                            ...
                            if cmp_date_due <= current_date:
                                ...
                                if (age.days >= 0 and age.days <= 30) or (due_date > report_date):
                                    if self.env.context.get('filter_original_currency'):
                                        final_columns.append(self._format(invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual)))
                                    else:
                    ...
                    if (not self.env.context.get('aging_due_filter_cmp')) or (due_date and (cmp_date_due <= current_date)):
                        ...
                        if self.env.context.get('filter_original_currency'):
                            vals['columns'].extend([self._format(invoice_sign * abs(aml.amount_residual_currency or aml.amount_residual)) or ''])
                            vals['columns'].extend([self._format(invoice_sign * abs(aml.amount_currency or aml.debit or aml.credit)) or ''])
                        else:
                ...
                for payment in payment_ids:
                    ...
                    if age.days >= 0 and age.days <= 30:
                        if self.env.context.get('filter_original_currency'):
                            final_columns.append(self._format(abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)))
                        else:
                    ...
                    vals['columns'].extend(final_columns)
                    if self.env.context.get('filter_original_currency'):
                        vals['columns'].extend([self._format(abs(move_line_id.amount_residual_currency or move_line_id.amount_residual)) or ''])
        ...
        if total and not line_id:
            ...
            if self.env.context.get('filter_original_currency'):
                total_line['columns'].extend([self._format(sign * sum([float("%.2f" % x) for x in partner_due_amount.values()]))])
