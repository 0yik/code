...
class ReportAccountFinancialReport(models.Model):
    ...
    def create_action_and_menu(self, parent_id):
        ...
        module = self._context.get('install_mode_data', {}).get('module', 'enterprise_accounting_report')
        IMD = self.env['ir.model.data']
        action_vals = {
            'name': self.get_title(),
            'tag': 'account_report_generic',
            'context': {
                'url': '/enterprise_accounting_report/output_format/financial_report/' + str(self.id),
    ...
    def get_report_type(self):
        return self.env.ref('enterprise_accounting_report.account_report_type_' + self.report_type)
...
class AccountFinancialReportLine(models.Model):
    ...
    type = fields.Selection([('account','Account'),('account_type','Account Type')], 'Type')
    ...
    def _get_with_statement(self, financial_report):
        ...
        if self.code not in ('CASHSTART', 'CASHEND') \
          and (financial_report == self.env.ref('enterprise_accounting_report.account_financial_report_cashsummary0') or self.env.context.get('cash_basis')):
