...
class report_account_general_ledger(models.AbstractModel):
    ...
    def do_query(self, line_id):
        ...
        if not line_id:
            ...
            other_account_ids = self.env['account.account'].search([('id','not in',account_ids)]).ids
            ...
            if account in context['context_id']['unfolded_accounts'] or unfold_all:
                ...
                for line in amls:
                    ...
                    domain_lines.append({
                        ...
                        'columns': [datetime.strptime(line.date, DEFAULT_SERVER_DATE_FORMAT).strftime(date_format),
...
class account_context_general_ledger(models.TransientModel):
    ...
    account_ids = fields.Many2many('account.account', string='Selected Accounts')

    @api.multi
    def get_available_account_ids_names_and_codes(self):
        return [[c.id, c.name, c.code] for c in self.env['account.account'].search([])]
    ...
    def get_columns_names(self):
        return [_("Date"), _("Reference"), _("Label"), _("Partner"), _("Debit"), _("Credit"), _("Balance")]
