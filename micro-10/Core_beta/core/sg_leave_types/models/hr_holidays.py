...
class hr_holiday_public(models.Model):
    ...
    #override method for stop mail notification.
    @api.multi
...
class hr_employee(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        if self._uid not in admin_group.users.ids:
...
#                 birth_date = datetime.datetime.strptime(obj.child_birthdate, DEFAULT_SERVER_DATE_FORMAT).date()
    ...
    def _check_is_admin(self):
        admin_group = self.env.ref('sg_leave_types.group_admin_allocate_leave')
        for obj in self:
            if obj._uid in admin_group.users.ids:
                obj.is_admin = True
    ...
    def _check_employee_leave(self):
        ...
        for rec in self:
            if rec.type == 'remove' and rec.holiday_status_id.pre_approved ==True:
                ...
                if qualify_date < datetime.datetime.today().date() and not rec.is_urgent:
    ...
    def action_approve(self):
        for holiday in self:
            holiday.with_context(tracking_disable=True).write({'state': 'validate'})
            holiday.action_validate()

            if holiday.is_recovery and holiday.number_of_days_temp == 0.0:
                if holiday.date_to and holiday.date_from:
                    least_date = self.onchange_date_to(date_from=holiday.date_from, date_to=holiday.date_to, half_day=False, holiday_status_id=holiday.holiday_status_id.id, employee_id=holiday.employee_id.id)
                    if least_date.get('value'):
                        #vals.update({'number_of_days_temp':least_date['value']['number_of_days_temp']})
                        if holiday.half_day:
                            holiday.number_of_days_temp = least_date['value']['number_of_days_temp'] 
                        else:
                            holiday.number_of_days_temp = least_date['value']['number_of_days_temp'] - 0.5

            if holiday.is_recovery and self._context.get('recovered_leave', False):
                domain2 = [
                ...
                for leave in ndholidays:
                    if holiday.number_of_days_temp != leave.number_of_days_temp:
                        leave.number_of_days_temp = leave.number_of_days_temp - holiday.number_of_days_temp
                    else:
                        leave.recovered_leave = True
    ...
    def onchange_date_from(self,date_from=False, date_to=False,half_day=False, holiday_status_id=False, employee_id=False):
        if date_from == False:
            if self.date_from and not self.half_day:
                ...
                tmp = frm_date + timedelta(hours=1)
                self.date_from = tmp + relativedelta(hour=1)
                date_from = self.date_from
#                 if self._context.get('default_is_recovery'):
#                     frm_date = datetime.datetime.strptime(self.date_from, DEFAULT_SERVER_DATETIME_FORMAT)
#                     if str(frm_date.time()) == "01:30:00":
#                         frm_date = frm_date - datetime.timedelta(minutes=30)
#                     self.date_from = frm_date + relativedelta(hour=1)
#                     date_from = self.date_from
#                 print date_from

            if self.date_from and self.half_day:
#                 if self._context.get('default_is_recovery'):
#                     frm_date = datetime.datetime.strptime(self.date_from, DEFAULT_SERVER_DATETIME_FORMAT)
#                     if str(frm_date.time()) == "01:00:00":
#                         frm_date = frm_date + datetime.timedelta(minutes=30)
#                     self.date_from = frm_date + relativedelta(hour=1)
#                     date_from = self.date_from

                if self.am_or_pm == 'AM':
                    date_f = str(datetime.datetime.strptime(self.date_from , DEFAULT_SERVER_DATETIME_FORMAT).date()) + " 00:00:00"
...
#             raise UserError(_('Warning!\nThe start date must be anterior to the end date.'))
        ...
        if self._context.get('default_is_recovery'):
            if not self.date_from:
                ...
                self.date_from = today + relativedelta(hour=1)
        return result
        ...
        if self.half_day and self.date_from and self.date_to:
            ...
            if self.am_or_pm == 'PM':
                if self.number_of_days_temp > 0.5:
                    ...
                    frm_date = datetime.datetime.strptime(self.date_from, DEFAULT_SERVER_DATETIME_FORMAT)
                    self.date_from = frm_date + relativedelta(hour=6)
#                     frm_date = frm_date + relativedelta(hour=5)
#                     self.date_from = frm_date - datetime.timedelta(minutes=30)
...
#         default.update({'date_from': str(date_from), 'date_to': str(date_to)})
    ...
    def _check_sg_maternity_leave_16_weeks(self):
        ...
        if self._context is None:
            self._context = {}
        for rec in self:
            if rec.type == 'remove' and rec.holiday_status_id.name in ['ML16', 'ML15', 'ML8', 'ML4', 'ML']:
                if rec.employee_id.gender == 'male':
                    raise ValidationError(_('Employee should be Female! \n This Leave is only applicable for Female !'))

                leave_type_id = self.env['hr.holidays.status'].search([('name', '=', 'ML')], limit=1)
                maternity_12week = self.env['hr.holidays'].search([('holiday_status_id', '=', leave_type_id.id), ('employee_id', '=', rec.employee_id.id), ('type', '=', 'remove'), ('id', '!=', rec.id)])
                ...
                if not maternity_12week and ((date_to - date_from).days + 1) != 84:
                    raise ValidationError(_('Warning.! \n First time you can request for 84 days for Maternity Leave. (12 weeks)'))
                if maternity_12week and ((date_to - date_from).days + 1) != 28:
                    raise ValidationError(_('Warning.! \n You can request for 28 days for Maternity Leave. (4 weeks)'))
...
#                         if rec.employee_id.singaporean == True and rec.employee_id.depends_singaporean == True:
                        if rec.employee_id.dependent_ids[0].singaporean:
                            ...
                            from_date = datetime.datetime.strptime(rec.date_from, DEFAULT_SERVER_DATETIME_FORMAT).date()
                            two_weeks = from_date - relativedelta(weeks=2)
                            if two_weeks < datetime.datetime.today().date():
                                raise ValidationError(_('Warning! \n Maternity Leave request should be submitted 2 weeks prior to the requested date.!'))
                        else:
