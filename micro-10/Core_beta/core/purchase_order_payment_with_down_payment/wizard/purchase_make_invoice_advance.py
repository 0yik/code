...
class PurchaseAdvancePaymentInv(models.TransientModel):
    ...
    _description = "Purchase Advance Payment Vendor Bill"
    ...
    advance_payment_method = fields.Selection([
        ...
        ], string='What do you want to Vendor Bill?', default=_get_advance_payment_method, required=True)
    product_id = fields.Many2one('product.product', string='Down Payment Product', domain=[('type', '=', 'service')],
        default=_default_product_id)
    count = fields.Integer(default=_count, string='# of Orders')
    amount = fields.Float('Down Payment Amount', digits=dp.get_precision('Account'), help="The amount to be vendor bill in advance, taxes excluded.")
    ...
    def create_invoices(self):
        ...
        if self.advance_payment_method == 'delivered':
            purchase_orders.action_bill_create()
        elif self.advance_payment_method == 'all':
            purchase_orders.action_bill_create(final=True)
        else:
            ...
            for order in purchase_orders:
                ...
                if self.product_id.invoice_policy != 'order':
                    raise UserError(_('The product used to bill a down payment should have an invoice policy set to "Ordered quantities". Please update your deposit product to be able to create a deposit invoice.'))
