        ...
        for record in self:
            ...
            for leave_type_id in leave_type_ids:
                ...
                if leave_type_bal_ids:
                    self._cr.execute(""" delete from hr_holidays_status_balance where id in %s""", (tuple(leave_type_bal_ids.sudo().ids),))
                ...
                for leave_waiting_id in leave_waiting_ids:
                    waiting_start_date_year = datetime.strptime(leave_waiting_id.date_from,DEFAULT_SERVER_DATETIME_FORMAT).year if leave_waiting_id.date_from else False
                    waiting_end_date_year = datetime.strptime(leave_waiting_id.date_to, DEFAULT_SERVER_DATETIME_FORMAT).year if leave_waiting_id.date_to else False
                ...
                leave_balance_dict['l_balance'] = record.id
...
#                     else:
#                         holiday_status_balance_rec = holiday_status_balance_obj.create(leave_balance_dict)
                    self._cr.execute('insert into hr_holidays_status_balance' '(employee_id,leave_name_l,leave_code,assigned_l,used_l,waiting_l,balance_l) values (%s,%s,%s,%s,%s,%s,%s)', (record.id,leave_type_id.id,leave_type_id.name,leave_assigned_count,leave_used_count,leave_waiting_count,leave_balance))
