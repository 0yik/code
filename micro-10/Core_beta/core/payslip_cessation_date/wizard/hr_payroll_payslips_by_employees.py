...
class HrPayslipEmployees(models.TransientModel):
    ...
    def compute_sheet(self):
        ...
        for employee in self.env['hr.employee'].browse(data['employee_ids']):
            ...
            elif employee.cessation_date and employee.cessation_date >= from_date and employee.cessation_date <= to_date:
                slip_data = self.env['hr.payslip'].with_context(date_from=from_date, date_to=employee.cessation_date, employee_id=employee.id, contract_id=False).onchange_employee()
                to_date = employee.cessation_date
            else:
        ...
        return {'type': 'ir.actions.act_window_close', 'skip_other':True}
