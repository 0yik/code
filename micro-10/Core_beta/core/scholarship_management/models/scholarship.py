...
class ScholarshipApplication(models.Model):
    ...
    def button_approve(self):
        available_qty = self.scholarship_id.available_qty
        if available_qty == 0 or available_qty < 0:
            raise UserError(_("There are no availability for this scholarship"))
        vals={}
        vals['student_id'] = self.student_id.id
        vals['pid'] = self.student_id.pid
        vals['standard_id'] = self.student_id.standard_id.id
        vals['year_id'] = self.student_id.year.id
        vals['scholarship_id'] = self.scholarship_id.id
        vals['start_date'] = self.student_id.year.date_start
        vals['end_date'] = self.student_id.year.date_stop
        self.env['scholarship.student'].create(vals)
