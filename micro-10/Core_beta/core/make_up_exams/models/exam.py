    ...
    def onchange_academic_year(self):
        ...
        for rec in self:
            if rec.academic_year:
                # To fill a Students Tab automatically based on Intake
                student_ids = student_obj.search([('year', '=',rec.academic_year.id),('state', '=', 'done'),('standard_id', '=', rec.academic_year.standard_id.id)])
                if student_ids:
                ...
                # To fill a Courses Tab automatically based on Intake 
                if rec.academic_year.standard_id.standard_id:
                    rec.standard_id = [(6, 0, [rec.academic_year.standard_id.standard_id.id])]
                else:
                    rec.standard_id = [(6, 0, [])]
