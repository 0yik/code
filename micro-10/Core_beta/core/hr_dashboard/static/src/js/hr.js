odoo.define('scs_hr_dashboard.update_kanban', function (require) {
"use strict";
...
KanbanView.include({
    employee_display_members_names: function() {
    ...
    on_groups_started: function() {
        ...
        if (self.dataset.model === 'hr.employee') {
            self.student_display_members_names();
        }
    },
});
...
KanbanRecord.include({
    on_card_clicked: function() {
        if (this.model === 'hr.employee') {
            this.$('.oe_kanban_hr_list a').first().click();
        } else {
