...
class PurchaseRequestLine(models.Model):
    ...
    def _compute_purchase_state(self):
        for rec in self:
            ...
            if rec.purchase_lines:
                ...
                elif any([po_line.state == 'confirmed' for po_line in rec.purchase_lines]):
    ...
    purchased_qty = fields.Float(string='Quantity in RFQ or PO', compute="_compute_purchased_qty")
    ...
    purchase_state = fields.Selection(compute="_compute_purchase_state", string="Purchase Status", selection=_PURCHASE_ORDER_LINE_STATE, store=True, default='none')
    ...
    def _calc_new_qty_price(self, request_line, po_line=None, cancel=False):
        ...
        qty = request_line.product_uom_id._compute_quantity(request_line.product_qty, request_line.product_id.uom_po_id)
        ...
        if po_line.order_id.picking_type_id.code != 'incoming':
            if po_line.product_id.seller_id.id == po_line.order_id.partner_id.id:
                ...
                supplierinfos = supplierinfo_obj.search([('name', '=', po_line.order_id.partner_id.id),('product_tmpl_id', '=', po_line.product_id.product_tmpl_id.id)])
                if supplierinfos:
        ...
        if supplierinfo_min_qty == 0.0:
            ...
            for rl in po_line.purchase_request_lines:
                qty += uom_obj._compute_qty(rl.product_uom_id.id, rl.product_qty, rl.product_id.uom_po_id.id)
            qty = max(qty, supplierinfo_min_qty) if qty > 0.0 else 0.0

        price = po_line.price_unit
        if qty != po_line.product_qty:
            pricelist_id = po_line.order_id.partner_id.property_product_pricelist.id
            pricelist_obj = self.env['product.pricelist'].browse(pricelist_id)
            price = pricelist_obj.price_get(request_line.product_id.id, qty, po_line.order_id.partner_id.id)[pricelist_id]
        return qty, price
    ...
    def unlink(self):
        for line in self:
            if line.purchase_lines:
                raise exceptions.Warning(_('You cannot delete a record that refers to purchase lines!'))
