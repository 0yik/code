        ...
        for rec in self:
            ...
            if rec.employee_id and rec.ot_date_from and rec.ot_date_to:
                ...
                for date_line in dates:
                    ...
                    emp_attendance_periods = self.env['hr_timesheet_sheet.sheet.periods'].sudo().search(
                        [('date', '=', date_line), ('sheet_id.employee_id', '=', rec.employee_id.id)])
                    if emp_attendance_periods:
                        ot1_hours += emp_attendance_periods.ot1_hours_manager
                        ot1_5_hours += emp_attendance_periods.ot1_5_hours_manager
                        ot2_hours += emp_attendance_periods.ot2_hours_manager
                    elif emp_attendance:
