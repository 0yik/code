...
class CrmReseller(FormatAddress, models.Model):

    _name = "crm.reseller"
    _description = "CRM Reseller"
    _order = "priority desc,date_action,id desc"
    _inherit = ['mail.thread', 'ir.needaction_mixin', 'utm.mixin']
    _mail_mass_mailing = _('CRM Reseller')
    ...
    name = fields.Char('Reseller', required=True, index=True)
    ...
    tag_ids = fields.Many2many('crm.lead.tag', 'crm_reseller_tag_rel', 'lead_id', 'tag_id', string='Tags', help="Classify and analyze your lead/opportunity categories like: Training, Service")
    ...
    lead_create_date = fields.Datetime('Create Date')
#     #this field is to store old migrated data
#     old_id = fields.Integer('Old id')
    ...
    def create(self, vals):
        ...
        return super(CrmReseller, self.with_context(context, mail_create_nolog=True)).create(vals)
    ...
    def write(self, vals):
        ...
        return super(CrmReseller, self).write(vals)
    ...
    def copy(self, default=None):
        ...
        return super(CrmReseller, self.with_context(context)).copy(default=default)
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(CrmReseller, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
        if view_type == 'form':
    ...
    def action_schedule_meeting(self):
        ...
        action['context'] = {
            'search_default_reseller_id': self.id if self.type == 'opportunity' else False,
            'default_reseller_id': self.id if self.type == 'opportunity' else False,
    ...
    def _mail_body(self, fields):
        """ generate the message body with the changed values
            ...
            :returns the body of the message for the current crm.reseller
        """
    ...
    def _merge_notify(self, opportunities):
        """ Create a message gathering merged leads/opps informations. Using message_post, send a
            ...
            resulting merge crm.reseller record.
            :param opportunities : recordset of merged crm.reseller
            :returns mail.message posted on resulting crm.reseller
        """
    ...
    def _merge_opportunity_history(self, opportunities):
        """ Move mail.message from the given opportunities to the current one. `self` is the
            crm.reseller record destination for message of `opportunities`.
            :param opportunities : recordset of crm.reseller to move the messages
        """
        self.ensure_one()
    ...
    def merge_opportunity(self, user_id=False, team_id=False):
        """ Merge opportunities in one. Different cases of merge:
            ...
            :return crm.reseller record resulting of th merge
        """
        if len(self.ids) <= 1:
    ...
    def get_empty_list_help(self, help):
        ...
        return super(CrmReseller, self.with_context(
    ...
    def retrieve_sales_dashboard(self):
        ...
        self._cr.execute("""
            SELECT
                ...
                l.type
            FROM mail_message M
                LEFT JOIN crm_reseller L ON (M.res_id = L.id)
                INNER JOIN crm_activity A ON (M.subtype_id = A.subtype_id)
            WHERE
                (M.model = 'crm.reseller') AND (L.user_id = %s) AND (L.type = 'opportunity')
        """, (self._uid,))
    ...
    def _track_subtype(self, init_values):
        ...
        return super(CrmReseller, self)._track_subtype(init_values)

    @api.multi
    def _notification_recipients(self, message, groups):
        """ Handle salesman recipients that can convert leads into opportunities
        and set opportunities as won / lost. """
        groups = super(CrmReseller, self)._notification_recipients(message, groups)
    ...
    def get_formview_id(self):
        if self.type == 'opportunity':
            ...
            view_id = super(CrmReseller, self).get_formview_id()
        return view_id

    @api.multi
    def message_get_suggested_recipients(self):
        recipients = super(CrmReseller, self).message_get_suggested_recipients()
        try:
            for lead in self:
                if lead.partner_id:
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        return super(CrmReseller, self).message_new(msg_dict, custom_values=defaults)
    ...
    def message_update(self, msg_dict, update_vals=None):
        ...
        return super(CrmReseller, self).message_update(msg_dict, update_vals=update_vals)

    @api.multi
    def message_partner_info_from_emails(self, emails, link_mail=False):
        result = super(CrmReseller, self).message_partner_info_from_emails(emails, link_mail=link_mail)
