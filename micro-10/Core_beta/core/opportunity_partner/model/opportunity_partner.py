...
class crm_lead(models.Model):
    ...
    def _onchange_user_id(self):
        ...
        if self._context.get('lead_id'):
            pipeline = self._context.get('lead_id') or False
            orders = self.env['sale.order'].search([('opportunity_id','=',pipeline)])
            for order in orders:
                self._cr.execute("""update sale_order set user_id=%s where id=%s"""%(self.user_id.id, order.id))
