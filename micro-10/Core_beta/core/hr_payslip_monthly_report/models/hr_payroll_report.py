...
class PayrollReportView(models.Model):
    ...
    def _from(self):
        from_str = """
            ...
            join hr_job jb on (emp.department_id=jb.id) join res_company cmp on (cmp.id=ps.company_id) JOIN hr_salary_rule hsr ON psl.salary_rule_id = hsr.id where hsr.appear_on_report=True 
         """
        return from_str

    def _group_by(self):
        group_by_str = """
            group by emp.id,psl.total,ps.date_from, ps.date_to, ps.state,jb.id,dp.id,cmp.id,hsr.name
        """
        return group_by_str
