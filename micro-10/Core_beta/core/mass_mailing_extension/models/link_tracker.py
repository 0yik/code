...
class link_tracker(models.Model):
    ...
    @api.one
    @api.depends('link_click_ids.link_id', 'link_click_ids')
    ...
    def _compute_redirected_url(self):
        ...
        self.redirected_url = '%s://%s%s?%s#%s' % (parsed.scheme, parsed.netloc, parsed.path, parsed.query, parsed.fragment)
    ...
    def action_view_statistics(self):
        action = self.env['ir.actions.act_window'].for_xml_id('mass_mailing_extension', 'action_view_click_statistics')
...
class MassMailing(models.Model):
    ...
    clicks_ratio = fields.Integer('Clicks Ratio', compute="_compute_clicks_ratio", store=True)
    clicks_count = fields.Integer('Clicks Ratio', compute="_compute_clicks_ratio", store=True)
    ...
    def _compute_clicks_ratio(self):
        ...
        if total_links:
            ...
            clicks_ratio = 100 * (float(click_count)/total_links)
        self.clicks_ratio = clicks_ratio
