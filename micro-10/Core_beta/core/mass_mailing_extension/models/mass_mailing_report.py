...
class MassMailingReport(models.Model):
    ...
    clicked = fields.Integer('Clicked', readonly=True)
    ...
    def init(self):
        ...
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW mail_statistics_report AS (
                ...
                GROUP BY ms.scheduled, utm_source.name, utm_campaign.name, mm.state, mm.email_from, mm.clicks_count
            )""")
