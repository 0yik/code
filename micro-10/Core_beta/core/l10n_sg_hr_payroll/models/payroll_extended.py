    ...
    def compute_sheet(self):
        ...
        for payslip in self:
            ...
            if employee and (employee.singaporean or employee.pr_year) and employee.employee_race_id:
                ...
                muslim_race = emp_race_obj.search([('name', '=', 'Muslim')], limit=1)
                if muslim_race and employee.employee_race_id and employee.employee_race_id.id == muslim_race.id and employee.cpf_shg_id:
                    for one_line in hr_payslip_line_obj.search(
                          [('slip_id', '=', payslip.id), ('salary_rule_id.race_id', '=', muslim_race.id)]):
                        if one_line.salary_rule_id.id != employee.cpf_shg_id.id:
...
class hr_employee(models.Model):
    ...
    identification_no = fields.Selection(selection=[('1', 'NRIC'),('2', 'FIN'), 
                                                    ...
                                                    ('6', 'Passport No. (for non-resident director and seaman only)')], default='1',
    ...
    cessation_provisions = fields.Selection(selection=[('Y', 'Cessation Provisions applicable'),
                                                       ('N', 'Cessation Provisions not applicable')], default='N', string='28. Cessation Provisions')
