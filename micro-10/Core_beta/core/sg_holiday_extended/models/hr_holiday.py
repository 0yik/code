    ...
    def create(self, vals):
        ...
        if 'half_day' in vals and vals['half_day'] ==True and 'type' in vals and vals['type'] =='remove':
            if 'date_from' in vals:
                date_to_with_delta = datetime.datetime.strptime(vals['date_from'], DEFAULT_SERVER_DATETIME_FORMAT) + datetime.timedelta(hours=4)
            ...
            if 'half_day' not in vals and rec.half_day == False:
                ...
                if vals['half_day'] ==True:
                    if 'date_from' in vals :
                        ...
                        date_to_with_delta = datetime.datetime.strptime(rec.date_from, DEFAULT_SERVER_DATETIME_FORMAT) + datetime.timedelta(hours=4)
                        vals.update({'date_to': date_to_with_delta,'number_of_days_temp':0.50})
...
#                        vals.update({'number_of_days_temp':least_date['value']['number_of_days_temp']})
    ...
    def _get_number_of_daystmp(self, date_from, date_to):
        ...
        from_dt = datetime.datetime.strptime(date_from, DEFAULT_SERVER_DATETIME_FORMAT)
        to_dt = datetime.datetime.strptime(date_to, DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def onchange_half_day(self):
        ...
        if self.half_day == True:
            ...
            if self.date_from:
                df = datetime.datetime.strptime(self.date_from, DEFAULT_SERVER_DATETIME_FORMAT) + relativedelta(hours=8)
                self.date_to = df
