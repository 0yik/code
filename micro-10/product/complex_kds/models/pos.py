...
class PosSalesOrder(models.Model):
    ...
    def confirm_sale(self, order_uid):
        orders = self.env['sale.order'].search([('origin', '=', order_uid)])
...
class PosOrder(models.Model):
    ...
    def create_from_ui(self, orders):
        orders = filter(lambda x: x['data'] and x['data']['lines'], orders)
...
class ResUsers(models.Model):
    ...
    def compare_pin_number_get_manager(self, pin_number):
        ...
        manager = self.search([('groups_id', 'in', group_pos_manager_id.id), ('pin_number', '=', pin_number)], limit=1)
