...
class SaleOrderExtension(models.Model):
    ...
    def check_order_line_qty(self):
        HTML = '<!DOCTYPE html><html><body>' \
               '<table cellspacing="50" width="100%" class="o_list_view table table-condensed table-striped">' \
               '<tr><th>Product</th><th>Required Quantity</th><th>Quantity On Hand</th></tr>'
        for line in self.order_line:
            if line.product_id and line.product_uom_qty:
                computed = line.compute_material_required()
                if computed != []:
                    HTML += '<tr><td>' + str(line.product_id.name) + '</td>'
                    HTML += '<td>' + str(line.product_uom_qty) + '</td>'
                    HTML += '<td>' + str(line.product_id.qty_available) + '</td>'
                    HTML += '</tr>'
    ...
    def action_confirm(self):
        ...
        for product in re_product_list:
            ...
            HTML += '</tr>'
        HTML += '</table></body></html>'
        unavailable_products = self.check_order_line_qty()
        ctx.update({
            'default_unavailable_products': unavailable_products,
