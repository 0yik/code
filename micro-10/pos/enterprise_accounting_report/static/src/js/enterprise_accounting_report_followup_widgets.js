odoo.define('enterprise_accounting_report.FollowupReportWidget', function (require) {
'use strict';
...
var FollowupReportWidget = ReportWidget.extend({
    events: _.defaults({
        ...
        'click .o_enterprise_accounting_report_followup_skip': 'skipPartner',
        'click .o_enterprise_accounting_report_followup_done': 'donePartner',
        'click .o_enterprise_accounting_report_followup-auto': 'enableAuto',
        "change *[name='blocked']": 'onChangeBlocked',
        'click .o_enterprise_accounting_report_followup-set-next-action': 'setNextAction',
    ...
    enableAuto: function(e) { // Auto mode is enabled -> Next action is computed automatically when clicking on 'Done'
        ...
        if ($(e.target).is('.btn-default')) { // If auto mode wasn't alreayd enabled
            target_id = $(e.target).parents("div.o_enterprise_accounting_report_page").data('context');
            ...
            $(e.target).parents("div.o_enterprise_accounting_report_page").find('.o_enterprise_accounting_report_followup-no-action').remove(); // Remove the alert stating that auto mode is disabled
        }
        else if ($(e.target).is('div.alert a')) { // If auto mode is enabled from the alert
            target_id = $("div.o_enterprise_accounting_report_page").data('context');
            // change which button is highlighted
            this.$("div.o_enterprise_accounting_report_page").find('div#followup-mode .o_enterprise_accounting_report_followup-auto').addClass('btn-info');
            this.$("div.o_enterprise_accounting_report_page").find('div#followup-mode .o_enterprise_accounting_report_followup-auto').removeClass('btn-default');
            this.$("div.o_enterprise_accounting_report_page").find('div#followup-mode .o_enterprise_accounting_report_followup-manual').addClass('btn-default');
            this.$("div.o_enterprise_accounting_report_page").find('div#followup-mode .o_enterprise_accounting_report_followup-manual').removeClass('btn-info');
            $(e.target).parents('.o_enterprise_accounting_report_followup-no-action').remove(); // Remove the alert
        }
    ...
    setNextAction: function(e) {
        e.stopPropagation();
        e.preventDefault();
        if ($(e.target).is('.o_enterprise_accounting_report_followup-manual.btn-default')){ // If manual mode isn't enabled yet
            $(e.target).toggleClass('btn-default btn-info'); // Change the highlighted buttons
            $(e.target).siblings().toggleClass('btn-default btn-info');
        }

        // Get the target and store it in a hidden field
        var target_id = $(e.target).parents("div.o_enterprise_accounting_report_page").data('context');
        var $content = $(QWeb.render("nextActionForm", {target_id: target_id}));
        var nextActionDatePicker = new datepicker.DateWidget(this);
        nextActionDatePicker.appendTo($content.find('div.o_enterprise_accounting_report_next_action_date_picker'));
        ...
        $content.find('.o_enterprise_accounting_report_followup_next_action_date_button').bind('click', changeDate);
        
        var save = function () {
            var note = $content.find(".o_enterprise_accounting_report_next_action_note").val().replace(/\r?\n/g, '<br />').replace(/\s+/g, ' ');
    ...
    onKeyPress: function(e) {
        var self = this;
        var report_name = $("div.o_enterprise_accounting_report_page").data("report-name");
            ...
            return new Model('account.report.context.followup.all').call('search', [[['create_uid', '=', session.uid]]]).then(function(result) {
                ...
                .filter([['id', '=', result[0]]]).first().then(function (result) {
                    if (result['partner_filter'] == 'action') {
                        ...
                        session.get_file({
                            url: '/enterprise_accounting_report/followup_report/' + letter_partner_list + '/',
                            complete: complete,
    ...
    printFollowupLetter: function(e) {
        ...
        if ($(e.target).data('primary') === 1) { // If letter printing was required
            $(e.target).parents('#action-buttons').addClass('o_enterprise_accounting_report_followup_clicked'); // Change the class to show the done button
    ...
    sendFollowupEmail: function(e) {
        e.stopPropagation();
        e.preventDefault();
        var context_id = $(e.target).parents("div.o_enterprise_accounting_report_page").attr("data-context");
        return new Model('account.report.context.followup').call('send_email', [[parseInt(context_id)]]).then (function (result) { // send the email server side
            $(e.target).parents("div.o_enterprise_accounting_report_page").find(".alert.alert-info.alert-dismissible").remove();
            if (result === true) {
                $(e.target).parents("div.o_enterprise_accounting_report_page").prepend(QWeb.render("emailSent")); // If all went well, notify the user
                if ($(e.target).data('primary') === 1) { // same as for letter printing
                    $(e.target).parents('#action-buttons').addClass('o_enterprise_accounting_report_followup_clicked');
                    $(e.target).toggleClass('btn-primary btn-default');
                    $(e.target).data('primary', '0');
                }
            }
            else {
                $(e.target).parents("div.o_enterprise_accounting_report_page").prepend(QWeb.render("emailNotSent")); // If something went wrong
            }
        });
    },
    displayExpNoteModal: function(e) {
        ...
        paymentDatePicker.appendTo($content.find('div.o_enterprise_accounting_report_payment_date_picker'));
        var save = function () {
    ...
    start: function() {
        ...
        var res2 = this.nextActionDatePicker.appendTo(this.$('div.o_enterprise_accounting_report_next_action_date_picker'));
