...
class AccountReportContextCommon(models.TransientModel):
	...
	def get_pdf(self):
		...
		body = self.env['ir.ui.view'].render_template(
			"enterprise_accounting_report.report_financial_letter",
	...
	def get_html_and_data(self, given_context=None):
		if given_context is None:
			given_context = {}
		result = {}
		if given_context.get('analytic_level_id'):
			self.date_filter_cmp = 'no_comparison'
		elif self.comparison:
			self.analytic_level_id = False
		if given_context:
		...
		# Setting context for Aging / Due filter
		if not given_context.get('aging_filter_cmp') and not given_context.get('aging_due_filter_cmp'):
			given_context.update({'aging_filter_cmp': True})

		# Setting context for Local / Original currency filter
		if ('filter_local_currency' not in given_context) and ('filter_original_currency' not in given_context):
			given_context.update({'filter_local_currency': True})

		# Passing the given_context to generate the report
		if self.get_report_obj().get_name() == 'aged_receivable':
			lines = self.with_context({'currency_ids':self.currency_ids.ids}).get_report_obj().with_context(**given_context).get_lines(self)
		else:
			lines = self.get_report_obj().with_context(**given_context).get_lines(self)
		rcontext = {
		...
		if currency_ids:
			for currency in currency_ids:
				if not filter_currency:
					filter_currency.append({
						'id':currency.id,
						'name':currency.name,
					})
				else:
					for fil in filter_currency:
						if fil['name'] != currency.name:
							filter_currency.append({
								'id':currency.id,
								'name':currency.name,
							})
		filter_currency = [i for n, i in enumerate(filter_currency) if i not in filter_currency[n + 1:]]
		if self.partner_ids:
			for iline in lines:
				if iline.get('id') in self.partner_ids.ids or iline.get('partnerid') in self.partner_ids.ids:
					filter_line.append(iline)
			rcontext = {
				...
				'report': self.get_report_obj(),
				'lines': filter_line,
				'footnotes': self.get_footnotes_from_lines(filter_line),
				'mode': 'display',
			}
		...
		if self.get_report_obj().get_name() == 'general_ledger':
			select += ['journal_ids', 'account_ids']
			result['available_journals'] = self.get_available_journal_ids_names_and_codes()
			result['available_accounts'] = self.get_available_account_ids_names_and_codes()
		...
		#Partner Filters
		result['available_partners'] = [(t['id'], t['name']) for t in lines if t.get('type',False) == 'partner_id']
		result['report_context']['partner_ids'] = self.partner_ids.ids

		#Currency Filters
		result['available_currency'] = filter_currency
		...
		if config_setting and config_setting.group_multi_currency:
			multi_currency = True
		result['report_context']['multi_currency'] = multi_currency,

		# Aging / Due aging filter
		if given_context and 'aging_filter_cmp' in given_context:
			result['report_context']['aging_filter_cmp'] = given_context['aging_filter_cmp']
		if given_context and 'aging_due_filter_cmp' in given_context:
			result['report_context']['aging_due_filter_cmp'] = given_context['aging_due_filter_cmp']
		# Local / Original currency filter
		if given_context and 'filter_local_currency' in given_context:
			result['report_context']['filter_local_currency'] = given_context['filter_local_currency']
		if given_context and 'filter_original_currency' in given_context:
			result['report_context']['filter_original_currency'] = given_context['filter_original_currency']
		if self.get_report_obj().get_name() == 'aged_receivable':
			result['report_context']['show_filter'] = True
	...
	def get_xlsx(self, response):
		...
		domain_style_right = workbook.add_format({'font_name': 'Arial', 'italic': True, 'right': 2, 'num_format': '#,##0.00'})
