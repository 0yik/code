...
class report_account_coa(models.AbstractModel):
    ...
    def _lines(self, line_id=None):
        ...
        for p in xrange(len(context['periods'])):
            debit_credit_total.update({p: [0.0, 0.0]})
        for account in sorted_accounts:
            ...
            debit_credit = sum([[grouped_accounts[account][p]['balance'] > 0 and self._format(grouped_accounts[account][p]['balance']) or '', grouped_accounts[account][p]['balance'] < 0 and self._format(-grouped_accounts[account][p]['balance']) or ''] for p in xrange(len(context['periods']))], [])
            initial_bal_total += (account in initial_balances and initial_balances[account] or 0.0)
            for p in xrange(len(context['periods'])):
                debit = debit_credit_total.get(p)[0] + (grouped_accounts[account][p]['balance'] > 0 and grouped_accounts[account][p]['balance'] or 0.0)
                credit = debit_credit_total.get(p)[1] + (grouped_accounts[account][p]['balance'] < 0 and -grouped_accounts[account][p]['balance'] or 0.0)
                debit_credit_total.update({p: [debit, credit]})
            to_amount = sum([[grouped_accounts[account][p]['balance'] > 0 and (grouped_accounts[account][p]['balance']) or '', grouped_accounts[account][p]['balance'] < 0 and (-grouped_accounts[account][p]['balance']) or ''] for p in xrange(len(context['periods']))], [])
            account_currency = account.currency_id or self.env.user.company_id.currency_id
            fx_debit = to_amount[0]
            fx_credit = to_amount[1]
            if to_amount[0]:
                fx_debit = self.env.user.company_id.currency_id.compute(to_amount[0],account_currency)
                fx_debit = formatLang(self.env, fx_debit, currency_obj=account_currency)
            if to_amount[1]:
                fx_credit = self.env.user.company_id.currency_id.compute(to_amount[1],account_currency)
                fx_credit = formatLang(self.env, fx_credit, currency_obj=account_currency)
            total = [self._format((account in initial_balances and initial_balances[account] or 0.0) + sum([grouped_accounts[account][p]['balance'] for p in xrange(len(context['periods']))]))]
            lines.append({
                'id': account.id,
                'type': 'account_id',
