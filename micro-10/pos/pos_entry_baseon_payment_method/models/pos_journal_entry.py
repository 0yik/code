    ...
    def action_pos_session_close(self):
        ...
        for session in self:
            ...
            for st in session.statement_ids:
                ...
                st.with_context(ctx).sudo().button_confirm_bank_pos()
...
class account_bank_statement_inh(models.Model):
        ...
        for statement in statements:
            ...
            for st_line in statement.line_ids:
                if st_line.account_id and not st_line.journal_entry_ids.ids:
                    st_line.fast_counterpart_creation_pos()
            ...
            for move in moves:
                for line in move.line_ids:
                    debit += line.debit
                    credit += line.credit
                if move == first_move and first_move:
                    remaining_moves = moves - move
            if first_move:
                for line in first_move.line_ids:
                    if line.debit > 0:
                        self.env.cr.execute("update account_move_line set debit=%s where id=%s",(debit,line.id))
                    if line.credit > 0:
                        self.env.cr.execute("update account_move_line set credit=%s where id=%s",(credit,line.id))
                if remaining_moves:
                    remaining_moves.unlink()
                first_move._amount_compute()
                first_move.filtered(lambda m: m.state != 'posted').post()
            statement.message_post(body=_('Statement %s confirmed, journal items were created.') % (statement.name,))
        statements.link_bank_to_partner()
        statements.write({'state': 'confirm', 'date_done': time.strftime("%Y-%m-%d %H:%M:%S")})
        

class account_bank_statement_line_inh(models.Model):
    
    _inherit = "account.bank.statement.line"
    
    def fast_counterpart_creation_pos(self):
        for st_line in self:
            ...
            st_line.process_reconciliation_pos(new_aml_dicts=[vals])
    
    def _get_communication(self, payment_method_id):
        return self.name or ''
    
    def process_reconciliation_pos(self, counterpart_aml_dicts=None, payment_aml_rec=None, new_aml_dicts=None):
