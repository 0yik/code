        ...
        for line_item in line.analytic_distribution_id:
            if line_item.rate:
                line_rate = line.analytic_distribution_id.search(
                    [('salary_rule_id', '=', line.id), ('analytic_level_id', '=', line_item.analytic_level_id.id)])
        ...
        for line in self.analytic_distribution_id:
            if line.rate:
                line_rate = self.analytic_distribution_id.search(
                    [('salary_rule_id', '=', self.id), ('analytic_level_id', '=', line.analytic_level_id.id)])
                amount = [a.rate for a in line_rate]
                if sum(amount) != 100:
                    raise ValidationError(
                        ("Sum of the rate should be 100 for analytic account level and related anlytic account"))
...
class HrContract(models.Model):
    ...
    def create(self, values):
        ...
        for line_item in line.analytic_distribution_id:
            if line_item.rate:
                line_rate = line.analytic_distribution_id.search(
                    [('hr_contract_id', '=', line.id), ('analytic_level_id', '=', line_item.analytic_level_id.id)])
                amount = [a.rate for a in line_rate]
                if sum(amount) != 100:
                    raise ValidationError(
                        ("Sum of the rate should be 100 for analytic account level and related anlytic account"))
    ...
    def write(self, values):
        ...
        for line in self.analytic_distribution_id:
            if line.rate:
                line_rate = self.analytic_distribution_id.search(
                    [('hr_contract_id', '=', self.id), ('analytic_level_id', '=', line.analytic_level_id.id)])
                amount = [a.rate for a in line_rate]
                if sum(amount) != 100:
                    raise ValidationError(
                        ("Sum of the rate should be 100 for analytic account level and related anlytic account"))
...
class HrExpense(models.Model):
    ...
    def create(self, values):
        ...
        for line_item in line.analytic_distribution_id:
            if line_item.rate:
                line_rate = line.analytic_distribution_id.search(
                    [('expense_id', '=', line.id), ('analytic_level_id', '=', line_item.analytic_level_id.id)])
                amount = [a.rate for a in line_rate]
                if sum(amount) != 100:
                    raise ValidationError(
                        ("Sum of the rate should be 100 for analytic account level and related anlytic account"))
    ...
    def write(self, values):
        ...
        for line in self.analytic_distribution_id:
            if line.rate:
                line_rate = self.analytic_distribution_id.search(
                    [('expense_id', '=', self.id), ('analytic_level_id', '=', line.analytic_level_id.id)])
                amount = [a.rate for a in line_rate]
                if sum(amount) != 100:
                    raise ValidationError(
                        ("Sum of the rate should be 100 for analytic account level and related anlytic account"))
