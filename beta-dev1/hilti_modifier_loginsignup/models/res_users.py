...
class res_users(models.Model):
    ...
    def verify_customer(self):
        user_template = self.env.ref('hilti_modifier_loginsignup.mail_template_successfull_verify_customer')
        for rec in self:
            if not rec.active:
                rec.write({
                    'active': True
                ...
                user_template.send_mail(rec.id, force_send=True)
        return True
...
#             }
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        if 'customer_verification' in self._context and self._context.get('customer_verification'):
            ...
            args.append(['id', 'in', self.env.ref('hilti_modifier_accessrights.group_hilti_customer').with_context({'active_test': False}).users.ids])
