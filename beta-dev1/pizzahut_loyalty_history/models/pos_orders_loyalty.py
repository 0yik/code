...
class PosOrder(models.Model):
	_inherit = 'pos.order'
	reward_id = fields.Many2one('loyalty.reward', string='Rewards Items')
	program_id = fields.Many2one('loyalty.program', string='Loyalty Programs')

	@api.model
	def create_from_ui(self,orders):
		order_ids = super(PosOrder,self).create_from_ui(orders)
		if order_ids['orders']:
			order_objs = self.env['pos.order'].browse(order_ids['orders'][0]['id'])
		else:
			order_objs = []
		result = {}
		order_list = []
		...
		for order_obj in order_objs:
			vals = {}
			vals['lines'] = []
			if orders[index].get('data').get('reward_id'):
				order_obj.reward_id = orders[index].get('data').get('reward_id')
			index +=1
			...
			vals['reward_id'] = order_obj.reward_id.id
