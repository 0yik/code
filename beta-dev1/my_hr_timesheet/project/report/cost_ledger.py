...
class account_analytic_cost_ledger(report_sxw.rml_parse):
    ...
    def _get_children(self, accounts):
        ...
        """
        analytic_obj = self.objects.env['account.analytic.account']
        res = set()
        for account in accounts:
            if account.id not in self.children:
                self.children[account.id] = analytic_obj.search([('parent_id', 'child_of', [account.id])])
            res.update(self.children[account.id])
        return tuple(res)

    def _lines_g(self, account, date1, date2):
        self._cr.execute("SELECT sum(aal.amount) AS balance, aa.code AS code, aa.name AS name, aa.id AS id \
    ...
    def _lines_a(self, general_account, account, date1, date2):
        self._cr.execute("SELECT aal.name AS name, aal.code AS code, aal.amount AS balance, aal.date AS date, aaj.code AS cj FROM account_analytic_line AS aal, account_analytic_journal AS aaj \
    ...
    def _sum_debit(self, accounts, date1, date2):
        self._cr.execute("SELECT sum(amount) FROM account_analytic_line WHERE account_id IN %s AND date>=%s AND date<=%s AND amount>0",
...
class report_analyticcostledger(models.AbstractModel):
    _name = 'report.hr_timesheet.report_analyticcostledger'
    _inherit = 'report.abstract_report'
    _template = 'hr_timesheet.report_analyticcostledger'
