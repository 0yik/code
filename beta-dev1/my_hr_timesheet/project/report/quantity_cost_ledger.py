...
class account_analytic_quantity_cost_ledger(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(account_analytic_quantity_cost_ledger, self).__init__(self, cr, uid, name, context)
    ...
    def _lines_g(self, account_id, date1, date2, journals):
        if not journals:
            self._cr.execute("SELECT sum(aal.unit_amount) AS quantity, \
    ...
    def _lines_a(self, general_account_id, account_id, date1, date2, journals):
        if not journals:
            self._cr.execute("SELECT aal.name AS name, aal.code AS code, \
...
class report_analyticcostledgerquantity(models.AbstractModel):
    _name = 'report.hr_timesheet.report_analyticcostledgerquantity'
    _inherit = 'report.abstract_report'
    _template = 'hr_timesheet.report_analyticcostledgerquantity'
