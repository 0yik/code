...
class account_inverted_analytic_balance(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(account_inverted_analytic_balance, self).__init__(self, cr, uid, name, context)
    ...
    def _lines_a(self, accounts, general_account_id, date1, date2):
        ids = map(lambda x: x.id, accounts)
        self._cr.execute("SELECT sum(aal.amount) AS balance, "
        ...
        aaa_obj = self.objects.env['account.analytic.account']
        res2 = aaa_obj.browse(ids)
        complete_name = {}
        for r in res2:
    ...
    def _sum_debit(self, accounts, date1, date2):
        ids = map(lambda x: x.id, accounts)
        self._cr.execute("SELECT sum(amount) \
    ...
    def _sum_quantity(self, accounts, date1, date2):
        ids = map(lambda x: x.id, accounts)
        self._cr.execute("SELECT sum(unit_amount) \
                FROM account_analytic_line \
                WHERE account_id IN %s AND date>=%s AND date<=%s", (tuple(ids),date1, date2,))
        return self.cr.fetchone()[0] or 0.0


class report_invertedanalyticbalance(models.AbstractModel):
    _name = 'report.hr_timesheet.report_invertedanalyticbalance'
    _inherit = 'report.abstract_report'
    _template = 'hr_timesheet.report_invertedanalyticbalance'
