...
class account_analytic_balance(report_sxw.rml_parse):
    ...
    def get_children(self, ids):
        read_data = self.objects.env['account.analytic.account'].browse(ids)
    ...
    def _lines_g(self, account_id, date1, date2):
        account_analytic_obj = self.objects.env['account.analytic.account']
        ids = account_analytic_obj.search(self.cr, self.uid,
                [('parent_id', 'child_of', [account_id])])
        self._cr.execute("SELECT aa.name AS name, aa.code AS code, \
    ...
    def _move_sum(self, account_id, date1, date2, option):
        if account_id not in self.acc_data_dict:
            account_analytic_obj = self.objects.env['account.analytic.account']
            ids = account_analytic_obj.search([('parent_id', 'child_of', [account_id])])
        ...
        if option == "credit":
            self._cr.execute("SELECT COALESCE(-sum(amount),0.0) FROM account_analytic_line \
            ...
            self._cr.execute("SELECT COALESCE(sum(unit_amount),0.0) FROM account_analytic_line \
                WHERE account_id IN %s\
    ...
    def _sum_all(self, accounts, date1, date2, option):
        ...
        if not self.acc_sum_list:
            ids2 = account_analytic_obj.search([('parent_id', 'child_of', ids)])
...
class report_analyticbalance(models.AbstractModel):
    _name = 'report.hr_timesheet.report_analyticbalance'
    _inherit = 'report.abstract_report'
    _template = 'hr_timesheet.report_analyticbalance'
