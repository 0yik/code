...
odoo.define('product_booked_date.book_date', function (require) {
    "use strict";
    ...
    function cal_init() {
		...
		ajax.jsonRpc("/booked", 'call', {'prod_id':sel_prod_id
	        ...
	        /*$('.apply-btn').click(function(evt) {
    			var price = eval($('div.js_product').find('span[itemprop="price"]')[0].innerHTML);
	            var rent_days = $('#rent_days').val();
	            if (rent_days) {
	            	var bkdays = rent_days.split('to');
	            	var from = bkdays[0].split("/");
	            	var f = new Date(from[2], from[1] - 1, from[0]);
	            	var to = bkdays[1].split("/");
	            	var t = new Date(to[2], to[1] - 1, to[0]);
	                ...
	                var rent_price = _calculate_rent_price(days, price);
	                $('div.js_product').find('.oe_price > span')[0].innerHTML = rent_price.toString();
	                $("#rental_price")[0].value = rent_price;
	                $('div.js_product').find("#rent_days").val($("#rent_days").val() + '  ( ' + days.toString() + ' days)');
	                $('#days').val(days.toString());
	                $('#warn_msg').css('display','none');
    ...
    $('document').ready(function(){
	    ...
	    function addScript()
		 { 
		    ...
		    script.src="/modifier_honos_theme/static/src/js/ccm_quickview.js";
		    var scriptwish=document.createElement('script');	    
		    scriptwish.type='text/javascript';	    
		    scriptwish.src="/honos_quick_view/static/src/js/quick_wish_compare.js"; 	   	    
		    $("head").append(script);	    
			...
			{                
				...
				$("script[src='/honos_quick_view/static/src/js/ccm_quickview.js']").remove()
				$("script[src='/honos_quick_view/static/src/js/quick_wish_compare.js']").remove()
		...
		{
			pid=pid;
			ajax.jsonRpc('/quickproductdata', 'call', {'product_id':pid}).then(function(data) {
		...
		$(".quick-view-a").off('click').on('click', function(){ 
        	...
        	if (!evt.isDefaultPrevented() && !$(this).is(".disabled")) {
        		...
        		ajax.jsonRpc("/booked", 'call', {'prod_id':sel_prod_id
                    ...
                    if (booked_days.length > 0){
                        $("#warningModal").modal();
                        $('.js_check_product').removeClass('active');
...
//        $('input.js_variant_change').change(function(evt){
//        	if ($('#rent_days').data('dateRangePicker')){
//		    	$('#rent_days').data('dateRangePicker').destroy();
//	    	}
//        	$('#rent_days').val('');
//        });
    ...
    $('.oe_website_sale').each(function () {
        ...
        $('.oe_cart').on('click', '.js_delete_product', function(e) {
            e.preventDefault();
            var adv_depo = $(this).closest('tr').next().find('#td-days span').attr('id');
            if (adv_depo) {
          	  var deposite_line = $(this).closest('tr').next().find('input.js_quantity').attr('data-line-id');
          	  ajax.jsonRpc("/advance_empty", 'call', {'line_id':deposite_line
        	    	}).then(function(res) {
