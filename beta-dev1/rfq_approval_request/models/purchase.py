    ...
    def shipment_notification_template(self):
        for order in self:
            ...
            if order.receive_date:
                formatted_date = datetime.strptime(order.receive_date, DEFAULT_SERVER_DATE_FORMAT).strftime('%d-%m-%Y')
...
#             order._add_supplier_to_product()
    ...
    def unreceived_notification_scheduler(self):
        for order in self.search([]):
            mail_vals = {}
            if order.receive_date == str(datetime.now())[:10] and not order.is_shipped and order.approval_id.user_id and not order.is_unreceived:
                partner_id = self.env['res.partner'].search([('name', '=', order.approval_id.user_id.name)])
                email_from = self.env.user.login
