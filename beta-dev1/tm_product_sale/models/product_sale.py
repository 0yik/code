...
class product_sale_report(models.Model):
    ...
    @api.multi
    def product_sale_report_print(self):
        domain = []
        if self.category_id:
        ...
        datas = {
            'doc_ids': self.ids,
            'doc_model': 'product.sale.report',
            'docs': self.read()[0]

        }
        return {
            'type': 'ir.actions.report.xml',
            'report_name': 'tm_product_sale.product_sale_report',
            'datas': datas
        }
class product_sale_report(models.AbstractModel):
    _name = 'report.tm_product_sale.product_sale_report'

    @api.multi
    def render_html(self, docids, data=None):
        report = self.env['report']._get_report_from_name('tm_product_sale.product_sale_report')
        stock_card = self.env['product.sale.report'].browse(docids)
        ...
        for product in products:
            ...
            for line in purchase_do:
                line_data = {
                    ...
                    # 'amount3'       : '{0:,.2f}'.format((saldo + line.product_uom_qty) * line.price_unit),
                }
                saldo = saldo + line.product_uom_qty
        ...
        return self.env['report'].render('tm_product_sale.product_sale_report', docargs)
