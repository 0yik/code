...
class PricelistItem(models.Model):
    ...
    compute_price = fields.Selection([
            ...
            ('price_discount', 'Discount (Fixed Amount)'),
            ('formula', 'Formula'),
            ('discount_induction', 'Discount(Induction)'),
            ('price_induction', 'Price(Induction)')], index=True, default='fixed')
    ...
    def _get_pricelist_item_name_price(self):
        ...
        elif self.compute_price == 'discount_induction':
            self.price = _("%s %% discount") % (self.percent_price)
        elif self.compute_price == 'price_induction':
            self.price = ("%s %s") % (self.fixed_price, self.pricelist_id.currency_id.name)
        elif self.compute_price == 'price_discount':
            self.price = ("%s %s") % (self.fixed_price, self.pricelist_id.currency_id.name)
        else:
    ...
    def _onchange_compute_price(self):
        ...
        if self.compute_price != 'price_induction':
            self.fixed_price = 0.0
            if self.compute_price != 'price_discount':
                self.fixed_price = 0.0
        if self.compute_price != 'discount_induction':
            self.percent_price = 0.0
        ...
        for product, qty, partner in products_qty_partner:
            ...
            for rule in items:
                ...
                if price is not False:
                    ...
                    elif rule.compute_price == 'discount_induction':
                        price = (price + (price * (rule.percent_price / 100))) or 0.0
                    elif rule.compute_price == 'price_induction':
                        price = (price + rule.fixed_price) or 0.0
                    elif rule.compute_price == 'price_discount':
                        price = (price - rule.fixed_price) or 0.0
                    else:
                        # complete formula
                        price_limit = price
