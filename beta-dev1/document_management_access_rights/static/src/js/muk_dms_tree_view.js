odoo.define('muk_dms.tree_view', function(require) {
    ...
    var open = function(self, model, id)
        {
            ...
            {
                var record_id = 'muk_dms_file_form_read';
            } else {
                var record_id = 'muk_dms_directory_form_read';
            }

            var IrModelData = new Model('ir.model.data', session.user_context);
            IrModelData.call('get_object_reference', ['document_management_access_rights',record_id]).then(function(result)
                {
                    self.do_action({
    ...
    var edit = function(self, model, id)
        {
            ...
            if (model == "muk_dms.file")
            {
                if (self.selected_node.data.perm_unlink){
                    var record_id = 'muk_dms_file_form_edit_delete';
                } else{
                    var record_id = 'muk_dms_file_form_edit';
                }
            } else {
                if (self.selected_node.data.perm_unlink){
                    var record_id = 'muk_dms_directory_form_edit_delete';
                } else{
                    var record_id = 'muk_dms_directory_form_edit';
                }
            }
	...
	var DMSTreeViewWidget = Widget.extend({
        	...
        	$.when(self.load_directories(self)).done(function (directories, directory_ids) {
        		$.when(self.load_files(self, directory_ids)).done(function (files) {
	        		...
	        		if (self.selected_node){
	        		    self.selected_node.state.selected = false;
                        self.selected_node.perm_create = true;
                        self.selected_node.perm_write = true;
                        self.selected_node.perm_read = true;
                        self.selected_node.perm_unlink = true;
	        		}
	        		self.$el.find('button.create_file').prop('disabled', false);
                    self.$el.find('button.create_directory').prop('disabled', false);
        ...
        load_directories: function(self) {
        	...
        	Directories.query(['name', 'parent_id', 'dir_perm_read', 'dir_perm_create',
        					   'dir_perm_write', 'dir_perm_unlink']).all().then(function(directories) {
        		var data = [];
        		...
        		_.each(directories, function(value, key, list) {
	        		data.push({
	        			...
	        			data: {
	        				...
	        				perm_read: value.dir_perm_read,
	        				perm_create: value.dir_perm_create,
	        				perm_write: value.dir_perm_write,
	        				perm_unlink: value.dir_perm_unlink,
	        			}
	        		});
	        	});
        ...
        load_files: function(self, directory_ids) {
        	...
        	Files.query(['filename', 'mime_type', 'file_extension', 'directory',
        	             'link_preview', 'link_download', 'file_size', 'file_perm_read',
        	             'file_perm_create', 'file_perm_write', 'file_perm_unlink']).all().then(function(files) {
        		var data = [];
        		_.each(files, function(value, key, list) {
	        		...
	        		data.push({
	        			...
	        			data: {
	        				...
	        				perm_read: value.file_perm_read,
	        				perm_create: value.file_perm_create,
	        				perm_write: value.file_perm_write,
	        				perm_unlink: value.file_perm_unlink,
	        			}
	        		});
	        	});
        ...
        create_file: function() {
        	if(this.selected_node) {
        	    ...
        	    create(this, "muk_dms.file", false);
        	}
        ...
        create_directory: function() {
        	if(this.selected_node) {
        	    ...
        	    create(this, "muk_dms.directory", false);
        	}
        },
	});
