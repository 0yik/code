...
class account_report_general_ledger(models.TransientModel):
    ...
    def print_ledgerreport_xls(self):
        ...
        body_header_list = ["DATE", "JRNL", "Partner", "Ref", "Move", "Entry Label", "Debit", "Credit", "Base Debit", "Base Credit", "Balance"]
        ...
        for i in range(1,15):
              ...
              column.width = 225 * 30
        body_body_list = ['ldate', 'lcode', 'partner_name', 'lref', 'move_name', 'lname', 'user_debit', 'user_credit', 'debit', 'credit', 'balance']
            ...
            for line in account['move_lines']:
                    ...
                    for item in body_body_list:
                        if item == 'debit':
                           ...
                           line[item] = formatLang(self.env, line[item], currency_obj=current_obj.company_id.currency_id)
                        elif item == 'user_debit':
                           line[item] = formatLang(self.env, line[item], currency_obj=current_obj.company_id.currency_id)
                        elif item == 'user_credit':
                           line[item] = formatLang(self.env, line[item], currency_obj=current_obj.company_id.currency_id)
                        elif item == 'balance':
                           line[item] = formatLang(self.env, line[item], currency_obj=current_obj.company_id.currency_id)
