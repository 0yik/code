...
class final_invoice_create(models.TransientModel):
    ...
    def do_create(self, context=None):
        ...
        ids = self.emv['account.analytic.line'].search([('invoice_id','=',False),('to_invoice','<>', False), ('account_id', 'in', context['active_ids'])])
        invs = self.env['account.analytic.line'].invoice_cost_create(data)
        mod_obj = self.env['ir.model.data']
        act_obj = self.env['ir.actions.act_window']
        mod_ids = mod_obj.search([('name', '=', 'action_invoice_tree1')])[0]
        res_id = mod_obj.read(mod_ids, ['res_id'])['res_id']
        act_win = act_obj.read([res_id])[0]
