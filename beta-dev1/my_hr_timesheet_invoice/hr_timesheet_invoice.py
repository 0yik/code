...
class account_analytic_account(models.Model):
    ...
    def on_change_partner_id(self):
        ...
        if self.partner_id and self.partner_id.id:
            part = self.env['res.partner'].browse(self.partner_id.id)
...
class account_analytic_line(models.Model):
    ...
    def _default_journal(self):
        ...
        record_ids = proxy.search([('user_id', '=', self._uid)])
        if record_ids:
    ...
    def _get_invoice_price(self, account, product_id, user_id, qty, context = {}):
        pro_price_obj = self.env['product.pricelist']
        if account.pricelist_id:
            pl = account.pricelist_id.id
            price = pro_price_obj.price_get(product_id, qty or 1.0,account.partner_id.id)[pl]
        else:
            price = 0.0
        return price

    def _prepare_cost_invoice(self, partner, company_id, currency_id, analytic_lines, context=None):
    ...
    def _prepare_cost_invoice_line(self, invoice_id, product_id, uom, user_id,
        ...
        if product_id:
            ...
            tax = self.pool['account.fiscal.position'].map_tax(account.partner_id.property_account_position, taxes)
    ...
    def invoice_cost_create(self, data=None, context=None):
        ...
        for (key_id, company_id, currency_id), analytic_lines in invoice_grouping.items():
            ...
            curr_invoice = self._prepare_cost_invoice(partner, company_id, currency_id, analytic_lines, context=context)
            ...
            last_invoice = invoice_obj.create(curr_invoice, context=invoice_context)
            ...
            for analytic_line in analytic_lines:
                ...
                if not analytic_line.to_invoice:
                    raise exceptions.Warning(_('Error!'), _('Trying to invoice non invoiceable line for %s.') % (analytic_line.product_id.name))
            ...
            for (product_id, uom, user_id, factor_id, account, journal_type), lines_to_invoice in invoice_lines_grouping.items():
                ...
                invoice_line_obj.create(curr_invoice_line, context=context)
            self.write([l.id for l in analytic_lines], {'invoice_id': last_invoice},)
