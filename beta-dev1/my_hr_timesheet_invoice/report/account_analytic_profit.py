...
class account_analytic_profit(report_sxw.rml_parse):
    ...
    def _journal_ids(self, form, user_id):
        ...
        ids=list(set([b.journal_id.id for b in lines]))
    ...
    def _line(self, form, journal_ids, user_ids):
        ...
        for line in lines:
            if line.account_id.pricelist_id:
                ...
                price=price_obj.price_get(line.product_id.id, line.unit_amount or 1.0, line.account_id.partner_id.id)[pl]
            else:
                name="/"
                discount=1.0
                id = -1
            ...
            if line.invoice_id:
                self._cr.execute('select id from account_analytic_line where invoice_id=%s', (line.invoice_id.id,))
                tot = 0
                for lid in self._cr.fetchall():
                    lid2 = line_obj.browse(lid[0])
                    pl=lid2.account_id.pricelist_id.id
                    price=price_obj.price_get(lid2.product_id.id, lid2.unit_amount or 1.0, lid2.account_id.partner_id.id)[pl]
