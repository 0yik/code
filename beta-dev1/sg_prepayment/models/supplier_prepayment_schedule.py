            ...
            for line in record.lines:
                if line.journal_id:
                    raise ValidationError(
                        'Error!\nCan not Compute because Voucher already generate with Number : ' + str(
...
class supplier_prepayment_schedule_line(models.Model):
    ...
    def do_payment(self):
        for line in self:
            ...
            payment_id.post()
            payment_id.write({'supplier_prepayment': True})
            for payment in payment_id.move_line_ids:
                line.write({'journal_id': payment.move_id.id,'supplier_prepayment': True})
        return True
