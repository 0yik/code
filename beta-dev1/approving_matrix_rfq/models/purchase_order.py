...
class purchase_order(models.Model):
    ...
    def _get_approval_matrix_line(self):
        for rec in self:
            ...
            if rec.approving_matrix_id:
                ...
                if rec.approving_matrix_id.apply_all==True:
    ...
    @api.depends('approving_matrix_line_ids')
    def _get_approval_matrix_line_ids(self):
        if self.approving_matrix_id.apply_all==False:
            l=self.approving_matrix_line_ids.filtered(lambda r: r.approved == False)
            if l:
                self.approverr=l[0].employee_ids.id
    ...
    approverr=fields.Integer(compute="_get_approval_matrix_line_ids")
    approver_flag=fields.Boolean(default=False)
    ...
    def request_rfq_approve(self):
        ...
        if lines:
            self.approver_flag=True
            self.state = 'waiting_for_approval'
        else:
            self.approver_flag=False
            self.state = 'rfq_approved'
            return True

        if self.approving_matrix_id.apply_all==True:
            for line in lines:
            ...
            mail_to=self.env['hr.employee'].search([('id','=',self.approverr)])
            user_ids = mail_to.mapped('user_id')
            if user_ids:
                self.send_mail_pr_approval_process(user_ids)
        ...
        for record in self:
            ...
            for line in record.approving_matrix_line_ids.filtered(lambda r: r.approved == False):
                if self.approving_matrix_id.apply_all==True:
                    ...
                    mail_to=self.env['hr.employee'].search([('id','=',self.approverr)])
                    user_ids = mail_to.mapped('user_id').ids
                    if self._uid in user_ids:
                        line.write({'approved':True})
                        if record.approving_matrix_line_ids.filtered(lambda r: r.approved == False):
                            self.write({'approverr':record.approving_matrix_line_ids.filtered(lambda r: r.approved == False)[0].employee_ids.id})
                        self.request_rfq_approve()
                        break
                    else:
                        raise Warning(_("You don't have access to approve this!"))
