...
class account_report_general_ledger(models.TransientModel):
    ...
    def print_ledgerreport_xls(self):
        ...
        body_header_list = ["ACCOUNT CODE","ACCOUNT","DATE", "JRNL", "Partner", "Ref", "Move", "Entry Label", "Debit", "Credit", "Balance"]
...
#             worksheet.write(row, col, formatLang(self.env, account['balance'], currency_obj=current_obj.company_id.currency_id), body_header_style)
            ...
            for line in account['move_lines']:
                    ...
                    row+=1
                    worksheet.write(row, col, account['code'], body_header_style)
                    worksheet.write(row, col+1, account['name'], body_header_style)
#                     worksheet.write(row, col+8, formatLang(self.env, account['debit'], currency_obj=current_obj.company_id.currency_id), body_header_style)
#                     worksheet.write(row, col+9, formatLang(self.env, account['credit'], currency_obj=current_obj.company_id.currency_id), body_header_style)
#                     worksheet.write(row, col+10, formatLang(self.env, account['balance'], currency_obj=current_obj.company_id.currency_id), body_header_style)
                    col =2
