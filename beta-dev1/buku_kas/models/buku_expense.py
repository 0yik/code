...
class BukuExpense(models.Model):
	_name = 'buku.expense'
	_rec_name = 'reference'
	...
	buku_expense_line_ids = fields.One2many('buku.expense.line','buku_expense_id',string='Bank')
	...
	def create(self,vals):
		...
		res = super(BukuExpense, self).create(vals)
		return res

	@api.multi
	def write(self,vals):
		total = 0
		res = super(BukuExpense, self).write(vals)
		for line in self.buku_expense_line_ids:
...
class BukuExpenseLine(models.Model):
	_name = 'buku.expense.line'

	amount_currency     = fields.Float(string='Amount Currency')
	buku_expense_id = fields.Many2one('buku.expense', string='Bank Transaction')
