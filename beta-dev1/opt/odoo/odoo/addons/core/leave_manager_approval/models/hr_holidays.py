...
class Hr_Holidays(models.Model):
    ...
    def _default_manager(self):
        res = self.env.user.employee_ids and self.env.user.employee_ids[0].parent_id.id or False
        if res:
            return res
        else:
    ...
    def onchange_employee(self):
        for record in self:
            record.next_manager_id = record.employee_id and record.employee_id.parent_id.id or self.env.user.id
    ...
    def action_draft(self):
        for holiday in self:
            ...
            holiday.sudo().write({
                'state': 'draft',
                'next_manager_id': self.employee_id and self.employee_id.parent_id.id or self.env.user.id,
                'total_approval': 0,
            })
            linked_requests = holiday.mapped('linked_request_ids')
            for linked_request in linked_requests:
                linked_request.sudo().action_draft()
            linked_requests.sudo().unlink()
        return True
    ...
    def action_approve(self):
        ...
        for holiday in self:
            if holiday.type == 'remove' and holiday.no_of_approval:
                if (holiday.next_manager_id) and (holiday.next_manager_id.user_id) and (self.env.user.id == holiday.next_manager_id.user_id.id):
                    holiday.sudo().write({'next_manager_id': self.env.user.employee_ids[0].parent_id.id,'total_approval': holiday.total_approval + 1})
                else:
                    raise UserError(_('Need to approve by '+ str(self.next_manager_id.name)))

                if (holiday.no_of_approval == holiday.total_approval) or (holiday.next_manager_id.id == False):
                    holiday.sudo().write({'state':'validate'})
    ...
    def action_validate(self):
        for holiday in self:
            if holiday.holiday_type == 'employee' and holiday.type == 'remove':
                ...
                holiday.sudo().write({'meeting_id': meeting.id})
