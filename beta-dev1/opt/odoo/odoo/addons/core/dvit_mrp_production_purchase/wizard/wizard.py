...
class WizMrpPurchase(models.TransientModel):
    ...
    def _prepare_po(self):
        ...
        production_id = self.env['mrp.production'].browse(self._context.get('active_id'))
        picking_type_id = self.env['stock.picking.type'].sudo().search([('warehouse_id','=',production_id.move_raw_ids and production_id.move_raw_ids[0].group_id and production_id.move_raw_ids[0].group_id.procurement_ids and production_id.move_raw_ids[0].group_id.procurement_ids[0].warehouse_id.id or False),('code','=','incoming')])
        if not picking_type_id:
            picking_type_id = production_id.picking_type_id
        p_vals = {
        'origin': production_id.origin or '' + ':' + production_id.name or '',
