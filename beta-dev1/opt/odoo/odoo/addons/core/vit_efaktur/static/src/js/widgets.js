odoo.define('web.inputmask_widgets', function (require) {
    ...
    var FieldMask = form_widgets.FieldChar.extend({
        ...
        render_value: function (mask) {
            this._super();
            if (this.attributes){
                if (this.attributes['data-inputmask'] && this.attributes['data-inputmask'].split(': ')[1]){
                    var val = this.attributes['data-inputmask'].split(': ')[1].slice(1, -1)
                    this.$el.inputmask(val);
                }
        ...
        store_dom_value: function () {
            if (this.$input && this.is_syntax_valid()) {
                var regex = /[-.]/g;
                this.internal_set_value(this.parse_value(this.$input.val().replace(regex, "")));
