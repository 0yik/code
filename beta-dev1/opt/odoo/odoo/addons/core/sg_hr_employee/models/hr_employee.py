...
class hr_employee(models.Model):
    ...
    def _compute_age(self):
        for records in self:
            years = 0
            if records.birthday:
                born = datetime.strptime(records.birthday, "%Y-%m-%d")
    ...
    def _get_rem_days(self):
        ...
        for employee in self:
            if employee.last_date and employee.emp_status == 'in_notice':
                from_dt = datetime.strptime(employee.last_date, "%Y-%m-%d")
...
#    parent_user_id2 = fields.Many2one(related='parent_id2.user_id', string="Indirect Manger User")
    ...
    def _default_get_cpf_muslim(self):
        return self.env['hr.salary.rule'].sudo().search([('code', '=', 'CPFMBMF')])
