var getOrgChartJobW;
getOrgChartJob = function(b, a) {
    this.config = { theme: "ula", color: "blue", enableEdit: true, enableZoom: true, enableSearch: true, enableMove: true, enableGridView: false, enableDetailsView: true, enablePrint: false, enableZoomOnNodeDoubleClick: true, scale: "auto", linkType: "M", orientation: getOrgChartJob.RO_TOP, primaryFields: ["Name", "Title"], photoFields: ["Image"], idField: null, parentIdField: null, secondParentIdField: null, levelSeparation: 100, siblingSeparation: 30, subtreeSeparation: 40, removeNodeEvent: "", updateNodeEvent: "", updatedEvent: "", insertNodeEvent: "", createNodeEvent: "", clickNodeEvent: "", renderNodeEvent: "", embededDefinitions: "", maxDepth: 30, dataSource: null, customize: [], expandToLevel: 3, boxSizeInPercentage: { minBoxSize: { width: 5, height: 5 }, boxSize: { width: null, height: null }, maxBoxSize: { width: 100, height: 100 } }, layout: null };
    var d = getOrgChartJob.util._4("colorScheme");
    if (d) { this.config.color = d }
    if (a) { for (var c in this.config) { if (typeof(a[c]) != "undefined") { this.config[c] = a[c] } } } this._d();
    this.version = "2.3.1";
    this.theme = getOrgChartJob.themes[this.config.theme];
    this.element = b;
    this.nodes = {};
    this._ak = [];
    this._ao = [];
    ...
    this._aN = null;
    this._au = [];
    this._zr = new getOrgChartJob.node(-1, null, null, null, 2, 2);
    ...
    for (id in this.config.customize) { if (this.config.customize[id].theme) { this.config.embededDefinitions += getOrgChartJob.themes[this.config.customize[id].theme].defs } } this._X = new getOrgChartJob._X(this.element);
    this._ax();
    this.load()
};
getOrgChartJob.prototype._ax = function() {
    this._f();
    var a = getOrgChartJob.INNER_HTML.replace("[theme]", this.config.theme).replace("[color]", this.config.color).replace(/\[height]/g, this._aL).replace(/\[toolbar-height]/g, this.theme.toolbarHeight);
    if (typeof(getOrgChartJobW) !== "undefined") {
        a = a.slice(0, -6);
        a += getOrgChartJobW
    }
    this.element.innerHTML = a
};
getOrgChartJob.prototype.resize = function() {
    this._f();
    this._X._t.style.height = this._aL + "px";
    this._X._u.style.height = this._aL + "px";
    this._X._7.style.height = this._aL + "px"
};
getOrgChartJob.prototype._f = function() {
...
getOrgChartJob.prototype.changeColorScheme = function(a) {
    if (this.config.color == a) { return } this._X._zB.className = this._X._zB.className.replace(this.config.color, a);
    this.config.color = a
};
getOrgChartJob.prototype._a5 = function() {
    this._ak = [];
    this._ao = [];
    this._zz = [];
    getOrgChartJob._F(this, this._zr, 0);
    getOrgChartJob._zk(this, this._zr, 0, 0, 0);
    getOrgChartJob._zD(this)
};
getOrgChartJob.prototype._zp = function(b, a) { if (this._ak[a] == null) { this._ak[a] = 0 } if (this._ak[a] < b.h) { this._ak[a] = b.h } };
getOrgChartJob.prototype._zQ = function(b, a) { if (this._ao[a] == null) { this._ao[a] = 0 } if (this._ao[a] < b.w) { this._ao[a] = b.w } };
getOrgChartJob.prototype._zA = function(b, a) { b.leftNeighbor = this._zz[a]; if (b.leftNeighbor != null) { b.leftNeighbor.rightNeighbor = b } this._zz[a] = b };
getOrgChartJob.prototype._P = function(a) {
    switch (this.config.orientation) {
        case getOrgChartJob.RO_TOP:
        case getOrgChartJob.RO_TOP_PARENT_LEFT:
        case getOrgChartJob.RO_BOTTOM:
        case getOrgChartJob.RO_BOTTOM_PARENT_LEFT:
            return a.w;
        case getOrgChartJob.RO_RIGHT:
        case getOrgChartJob.RO_RIGHT_PARENT_TOP:
        case getOrgChartJob.RO_LEFT:
        case getOrgChartJob.RO_LEFT_PARENT_TOP:
            return a.h
    }
    return 0
};
getOrgChartJob.prototype._O = function(g, d, e) { if (d >= e) { return g } if (g._Y() == 0) { return null } var f = g._Y(); for (var a = 0; a < f; a++) { var b = g._G(a); var c = this._O(b, d + 1, e); if (c != null) { return c } } return null };
getOrgChartJob.prototype._A = function() {
    var e = [];
    var g;
    if (this._X._v) { g = getOrgChartJob.util._5(this._X) } else { g = this._6() } e.push(getOrgChartJob.OPEN_SVG.replace("[defs]", this.config.embededDefinitions).replace("[viewBox]", g.toString()));
    ...
    e.push(getOrgChartJob.buttons.draw());
    e.push(getOrgChartJob.CLOSE_SVxG);
    return e.join("")
};
getOrgChartJob.prototype._r = function(a, i, c, b, g, h) { var d = a; var f = null; if (i) { f = i * ((this._aP / 100) / (b.w + g)) } var e = null; if (c) { e = c * ((this._aL / 100) / (b.h + h)) } if (f != null && e != null) { d = f > e ? e : f } else { if (f != null) { d = f } else { if (e != null) { d = e } } } return d };
getOrgChartJob.prototype._6 = function() {
    ...
    switch (this.config.orientation) {
        ...
        case getOrgChartJob.RO_TOP_PARENT_LEFT:
            ...
            if (this.scale === "auto") {
                ...
                if (this.config.orientation == getOrgChartJob.RO_TOP) { a = c.x - o / 2 + c.w / 2 }
            }
        ...
        case getOrgChartJob.RO_BOTTOM_PARENT_LEFT:
            ...
            if (this.scale === "auto") {
                ...
                o = (this._aP) / this.scale;
                d = (this._aL) / this.scale;
                if (this.config.orientation == getOrgChartJob.RO_BOTTOM) { a = c.x - o / 2 + c.w / 2 }
            }
        ...
        case getOrgChartJob.RO_RIGHT_PARENT_TOP:
            ...
            if (this.scale === "auto") {
                ...
                o = (this._aP) / this.scale;
                d = (this._aL) / this.scale;
                if (this.orientation == getOrgChartJob.RO_RIGHT) { b = c.y - d / 2 + c.h / 2 }
            }
        ...
        case getOrgChartJob.RO_LEFT_PARENT_TOP:
            ...
            if (this.scale === "auto") {
                ...
                o = (this._aP) / this.scale;
                d = (this._aL) / this.scale;
                if (this.config.orientation == getOrgChartJob.RO_LEFT) { b = c.y - d / 2 + c.h / 2 }
            }
...
getOrgChartJob.prototype.draw = function(a) {
    this._X._a8();
    this._a5();
...
getOrgChartJob.prototype._z = function(a) {
    var g = [];
    ...
    for (var c = 0; c < g.length; c++) {
        ...
        var b = getOrgChartJob.util._3(f);
        var h = b.slice(0);
        h[4] = e.x;
        h[5] = e.y;
...
getOrgChartJob.prototype._aW = function(c, b) {
...
getOrgChartJob.prototype._aS = function(b, a) {
    this._ze(b, "mouseup", this._aS);
    this._ze(b, "mouseleave", this._aS);
    clearInterval(b.interval)
};
getOrgChartJob.prototype.move = function(f, a, b) {
    var h = getOrgChartJob.util._5(this._X);
    var e = h.slice(0);
...
getOrgChartJob.prototype.isCollapsed = function(a) { if ((a.parent == this._zr) || (a.parent == null)) { return false } if (a.parent.collapsed != getOrgChartJob.EXPANDED) { return true } else { return this.isCollapsed(a.parent) } return false };
getOrgChartJob.prototype._e = function() {
...
getOrgChartJob.prototype._q = function(b, c, d, e) {
    if (!e) { e = "" }
...
getOrgChartJob.prototype._ze = function(a, b) {
...
getOrgChartJob.prototype._zN = function(b, a) { if (!this._E) { this._E = {} } if (!this._E[b]) { this._E[b] = new Array() } this._E[b].push(a) };
getOrgChartJob.prototype._d = function() { if (this.config.removeNodeEvent) { this._zN("removeNodeEvent", this.config.removeNodeEvent) } if (this.config.updateNodeEvent) { this._zN("updateNodeEvent", this.config.updateNodeEvent) } if (this.config.createNodeEvent) { this._zN("createNodeEvent", this.config.createNodeEvent) } if (this.config.clickNodeEvent) { this._zN("clickNodeEvent", this.config.clickNodeEvent) } if (this.config.renderNodeEvent) { this._zN("renderNodeEvent", this.config.renderNodeEvent) } if (this.config.insertNodeEvent) { this._zN("insertNodeEvent", this.config.insertNodeEvent) } if (this.config.updatedEvent) { this._zN("updatedEvent", this.config.updatedEvent) } };
getOrgChartJob.prototype._R = function(b, a) { if (!this._E) { return true } if (!this._E[b]) { return true } var d = true; if (this._E[b]) { var c; for (c = 0; c < this._E[b].length; c++) { if (this._E[b][c](this, a) === false) { d = false } } } return d };
getOrgChartJob._X = function(a) {
    this.element = a;
    this._n
};
getOrgChartJob._X.prototype._a8 = function() {
...
getOrgChartJob._X.prototype._a7 = function() {
...
getOrgChartJob._X.prototype._zg = function(a) {
    this._t.style.overflow = "auto";
    this._v.style.width = (a + "px")
};
getOrgChartJob._X.prototype._U = function() { return this._i.getElementsByTagName("input")[0] };
getOrgChartJob._X.prototype._H = function() {
...
getOrgChartJob._X.prototype._J = function() { return this._i.getElementsByTagName("input") };
getOrgChartJob._X.prototype._N = function() { var a = this._i.getElementsByTagName("select"); for (i = 0; i < a.length; i++) { if (a[i].className == "get-oc-labels") { return a[i] } } return null };
getOrgChartJob._X.prototype._M = function() { var a = this._i.getElementsByTagName("select"); for (i = 0; i < a.length; i++) { if (a[i].className == "get-oc-select-parent") { return a[i] } } return null };
getOrgChartJob._X.prototype.getNodeById = function(a) { return this._a4.querySelector("[data-node-id='" + a + "']") };
getOrgChartJob._X.prototype.removeLinks = function() { var a = this._a4.querySelectorAll("[data-link-id]"); var b = a.length; while (b--) { a[b].parentNode.removeChild(a[b]) } };
getOrgChartJob._X.prototype.getButtonByType = function(a) { return this._a4.querySelector("[data-btn-action='" + a + "']") };
getOrgChartJob._X.prototype._zT = function(a) {
    var c;
...
getOrgChartJob.RO_TOP = 0;
getOrgChartJob.RO_BOTTOM = 1;
getOrgChartJob.RO_RIGHT = 2;
getOrgChartJob.RO_LEFT = 3;
getOrgChartJob.RO_TOP_PARENT_LEFT = 4;
getOrgChartJob.RO_BOTTOM_PARENT_LEFT = 5;
getOrgChartJob.RO_RIGHT_PARENT_TOP = 6;
getOrgChartJob.RO_LEFT_PARENT_TOP = 7;
getOrgChartJob.OPEN_SVG = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="[viewBox]"><defs>[defs]</defs><g>';
getOrgChartJob.CLOSE_SVG = "</svg>";
getOrgChartJob.OPEN_NODE = '<g style="cursor:pointer;" data-node-id="[data-node-id]" class="get-level-[level] [nodeCssClass]" transform="matrix(1,0,0,1,[x],[y])">';
getOrgChartJob.CLOSE_NODE = "</g>";
getOrgChartJob.NOT_DEFINED = 0;
getOrgChartJob.COLLAPSED = 1;
getOrgChartJob.EXPANDED = 2;
getOrgChartJob.MIXED_HIERARCHY_RIGHT_LINKS = 0;
getOrgChartJob._F = function(h, g, d) {
    var c = null;
    g.x = 0;
    g.y = 0;
    g._a9 = 0;
    ...
    h._zQ(g, d);
    h._zA(g, d);
    if (g.collapsed == getOrgChartJob.NOT_DEFINED) { g.collapsed = (h.config.expandToLevel && h.config.expandToLevel <= d && g._Y() ? getOrgChartJob.COLLAPSED : getOrgChartJob.EXPANDED) }
    if (g._Y() == 0 || d == h.config.maxDepth) { c = g._L(); if (c != null) { g._a9 = c._a9 + h._P(c) + h.config.siblingSeparation } else { g._a9 = 0 } } else {
        var f = g._Y();
        for (var a = 0; a < f; a++) {
            var b = g._G(a);
            getOrgChartJob._F(h, b, d + 1)
        }
        var e = g._B(h);
        e -= h._P(g) / 2;
        c = g._L();
        if (c != null) {
            ...
            g._al = g._a9 - e;
            getOrgChartJob._w(h, g, d)
        } else { if (h.config.orientation <= 3) { g._a9 = e } else { g._a9 = 0 } }
    }
};
getOrgChartJob._w = function(t, m, g) {
    var a = m._I();
...
getOrgChartJob._zk = function(h, d, b, i, k) {
    if (b <= h.config.maxDepth) {
        ...
        if (d._Y() != 0) { getOrgChartJob._zk(h, d._I(), b + 1, i + d._al, k + c + h.config.levelSeparation) }
        var f = d._2();
        if (f != null) { getOrgChartJob._zk(h, f, b, i, k) }
    }
};
getOrgChartJob._zD = function(e) {
    e._zf = e._zr.x;
    e._zv = e._zr.y;
    if (e._aN) {
...
getOrgChartJob.node = function(d, f, h, c, g, e, a, b) {
    this.id = d;
    this.pid = f;
    this.spid = h;
...
getOrgChartJob.node.prototype.getParent = function() { if (this.displayParent != null) { return this.displayParent } return this.parent };
getOrgChartJob.node.prototype.getChildren = function() { if (this.displayChildren != null) { return this.displayChildren } var a = []; for (i = 0; i < this.children.length; i++) { if (this.children[i].displayParent == null) { a.push(this.children[i]) } } return a };
getOrgChartJob.node.prototype.getImageUrl = function() { if (this.photoFields && this.data[this.photoFields[0]]) { return this.data[this.photoFields[0]] } return null };
getOrgChartJob.node.prototype._Y = function() { if (this.displayChildren == null && this.collapsed == getOrgChartJob.COLLAPSED) { return 0 } else { if (this.getChildren() == null) { return 0 } else { return this.getChildren().length } } };
getOrgChartJob.node.prototype._L = function() { if (this.leftNeighbor != null && this.leftNeighbor.getParent() == this.getParent()) { return this.leftNeighbor } else { return null } };
getOrgChartJob.node.prototype.isVisible = function() { if (this.x == 0 && this.y == 0) { return false } return true };
getOrgChartJob.node.prototype._2 = function() { if (this.rightNeighbor != null && this.rightNeighbor.getParent() == this.getParent()) { return this.rightNeighbor } else { return null } };
getOrgChartJob.node.prototype._G = function(a) { return this.getChildren()[a] };
getOrgChartJob.node.prototype._B = function(a) {
    node = this._I();
    node1 = this._K();
    return node._a9 + ((node1._a9 - node._a9) + a._P(node1)) / 2
};
getOrgChartJob.node.prototype._I = function() { return this._G(0) };
getOrgChartJob.node.prototype._K = function() { return this._G(this._Y() - 1) };
getOrgChartJob.node.prototype._Z = function(a) {
    ...
    var f = 0,
        ...
        o = 0;
    var c = null;
    var e = a.customize[this.id] && a.customize[this.id].theme ? getOrgChartJob.themes[a.customize[this.id].theme] : getOrgChartJob.themes[a.theme];
    ...
    for (var b = 0; b < this.getChildren().length; b++) {
        ...
        if (this.displayChildren == null && c.displayChildren != null && this.collapsed == getOrgChartJob.EXPANDED) {
            switch (a.orientation) {
                case getOrgChartJob.RO_TOP:
                case getOrgChartJob.RO_TOP_PARENT_LEFT:
                case getOrgChartJob.RO_BOTTOM:
            ...
            if (this.displayChildren != null) { d.push('<path data-link-id="' + this.id + '" class="link"   d="M' + f + "," + l + " " + g + "," + m + " " + h + "," + n + " L" + j + "," + o + '"/>') } else {
                if (this.collapsed == getOrgChartJob.EXPANDED) {
                    switch (a.linkType) {
                        case "M":
        ...
        if (a.expandToLevel && this.displayChildren == null) { d.push(getOrgChartJob.buttons.expColl.replace("[display]", this.collapsed == getOrgChartJob.EXPANDED ? "none" : "block").replace(/\[xa]/g, (p - e.expandCollapseBtnRadius)).replace(/\[ya]/g, (u - e.expandCollapseBtnRadius)).replace(/\[start]/g, ((e.expandCollapseBtnRadius * 2) / 6)).replace(/\[middle]/g, (e.expandCollapseBtnRadius)).replace(/\[end]/g, ((e.expandCollapseBtnRadius * 2 / 6) * 5)).replace(/\[id]/g, this.id)) }
    }
    return d.join("")
};
getOrgChartJob.node.prototype._W = function(c) {
    ...
    var h = 0,
        ...
        b = 0;
    var e = null;
    var g = c.customize[this.id] && c.customize[this.id].theme ? getOrgChartJob.themes[c.customize[this.id].theme] : getOrgChartJob.themes[c.theme];
    ...
    for (var d = this.secondChildren.length - 1; d >= 0; d--) {
        ...
        if (e.parent.collapsed != getOrgChartJob.EXPANDED) { continue }
        switch (c.orientation) {
            case getOrgChartJob.RO_TOP:
            case getOrgChartJob.RO_TOP_PARENT_LEFT:
                j = e.x + (e.w / 2);
        ...
        if (this.collapsed == getOrgChartJob.EXPANDED) { f.push('<path data-link-id="' + this.id + '" class="link-second-parent"   d="M' + a + "," + b + "  L" + j + "," + m + '"/>') }
    }
    return f.join("")
};
getOrgChartJob.node.prototype.getMostDeepChild = function(c) {
    var b = this;

    function a(f, g) { if (f.collapsed == getOrgChartJob.EXPANDED || f.displayChildren != null) { for (var d = 0; d < f.getChildren().length; d++) { var e = g[f.getChildren()[d].id]; if (e.level > b.level) { b = e } a(f.getChildren()[d], g) } } } a(this, c);
    return b
};
getOrgChartJob.node.prototype.draw = function(a) {
    var h = [];
    var b = this.getImageUrl();
    var m = a.customize[this.id] && a.customize[this.id].theme ? getOrgChartJob.themes[a.customize[this.id].theme] : getOrgChartJob.themes[a.theme];
    ...
    h.push(getOrgChartJob.OPEN_NODE.replace("[data-node-id]", this.id).replace("[x]", this._zU == null ? this.x : this._zU).replace("[y]", this._zJ == null ? this.y : this._zJ).replace("[level]", this.level).replace("[nodeCssClass]", d));
...
if (typeof(get) == "undefined") { get = {} } get._z = function(a, c, b, h, j, d) {
    ...
    function f() {
        for (var s in b) {
            var t = getOrgChartJob.util._s(["top", "left", "right", "bottom"], s.toLowerCase()) ? "px" : "";
...
if (typeof(get) == "undefined") { get = {} } get._c = function() {
    ...
    getOrgChartJob._c = { msie: a.browser == "msie", webkit: a.browser == "webkit", mozilla: a.browser == "mozilla", opera: a.browser == "opera" };
    return getOrgChartJob._c
};
getOrgChartJob.util = {};
getOrgChartJob.util._5 = function(_X) {
    var viewBox = _X._v.getAttribute("viewBox");
    viewBox = "[" + viewBox + "]";
    return eval(viewBox.replace(/\ /g, ", "))
};
getOrgChartJob.util._3 = function(element) {
    var transform = element.getAttribute("transform");
    transform = transform.replace("matrix", "").replace("(", "").replace(")", "");
    transform = getOrgChartJob.util._zV(transform);
    transform = "[" + transform + "]";
    return eval(transform.replace(/\ /g, ", "))
};
getOrgChartJob.util._zV = function(a) { return a.replace(/^\s+|\s+$/g, "") };
getOrgChartJob.util._s = function(a, c) { var b = a.length; while (b--) { if (a[b] === c) { return true } } return false };
getOrgChartJob.util._V = function(b) { var a = "1"; while (b[a]) { a++ } return a };
getOrgChartJob.util._4 = function(f) {
    var h = [],
        c;
...
getOrgChartJob.util._zS = function(c) {
...
getOrgChartJob.util._ay = function(a) { if (a == null || typeof(a) == "undefined" || a == "" || a == -1) { return true } return false };
getOrgChartJob.util._ag = function(a) { return (typeof a !== "undefined" && a !== null) };
getOrgChartJob.util._a1 = function(b, a) { var c = b.getBoundingClientRect(); var d = a.touches[0].pageX - c.left; var f = a.touches[0].pageY - c.top; var g = a.touches[1].pageX - c.left; var h = a.touches[1].pageY - c.top; return Math.sqrt((d - g) * (d - g) + (f - h) * (f - h)) };
getOrgChartJob.util._a3 = function(b, a) { var c = b.getBoundingClientRect(); var g = a.touches[0].pageX - c.left; var h = a.touches[0].pageY - c.top; var i = a.touches[1].pageX - c.left; var j = a.touches[1].pageY - c.top; var d = ((g - i) / 2 + i) / (c.width / 100); var f = ((h - j) / 2 + j) / (c.height / 100); return [d, f] };
getOrgChartJob.util._aZ = function(a, b, c) { var d = a.getBoundingClientRect(); var g = b - d.left; var h = c - d.top; var e = (g) / (d.width / 100); var f = (h) / (d.height / 100); return [e, f] };
getOrgChartJob.util._zF = function(b, a) { var c = b.getBoundingClientRect(); var d = a.touches[0].pageX - c.left; var f = a.touches[0].pageY - c.top; return Math.sqrt((d - t2x) * (d - t2x) + (f - t2y) * (f - t2y)) };
getOrgChartJob.util._1 = function(a) {
...
getOrgChartJob.prototype.showDetailsView = function(d) {
    var h = this.nodes[d];
    ...
    var a = function(l, k) {
        ...
        var m;
        for (i = 0; i < k.length; i++) { if (!getOrgChartJob.util._s(l, k[i])) { m += '<option value="' + k[i] + '">' + k[i] + "</option>" } } n += m;
        n += "</select>";
        if (!m) { n = "" }
        return n
    };
    ...
    c += getOrgChartJob.DETAILS_VIEW_ID_INPUT.replace("[personId]", h.id);
    for (label in h.data) {
        c += getOrgChartJob.DETAILS_VIEW_INPUT_HTML.replace(/\[label]/g, label).replace("[value]", h.data[label]);
        g.push(label)
    }
    c += a(g, this._au);
    this._X._i.innerHTML = c;
    var e = h.getImageUrl ? h.getImageUrl() : "";
    if (e) { this._X._m.innerHTML = getOrgChartJob.DETAILS_VIEW_ID_IMAGE.replace("[src]", e) } else { this._X._m.innerHTML = getOrgChartJob.DETAILS_VIEW_USER_LOGO } this._k();
    var j = this.config.customize[h.id] && this.config.customize[h.id].theme ? getOrgChartJob.themes[this.config.customize[h.id].theme].toolbarHeight : this.theme.toolbarHeight;
...
getOrgChartJob.prototype._k = function() { var a = this._X._J(); if (this._X._N()) { this._q(this._X._N(), "change", this._j) } };
getOrgChartJob.prototype._j = function(l, a) {
    var m = this._X._H();
    var k = this._X._N();
    var h = k.value;
    ...
    var e = getOrgChartJob.DETAILS_VIEW_INPUT_HTML.replace(/\[label]/g, h).replace("[value]", "");
...
getOrgChartJob.prototype._zt = function(e, a) {
    if (this._S) {
...
getOrgChartJob.prototype._zZ = function() { this.showGridView() };
getOrgChartJob.prototype.showGridView = function() {
...
getOrgChartJob.prototype._zW = function(b, a) { this.showMainView() };
getOrgChartJob.prototype.showMainView = function() {
...
getOrgChartJob.prototype._zs = function(b, a) { this.print() };
getOrgChartJob.prototype.print = function() {
    var b = this,
        d = this._X.element,
        k = this._X._zC,
        g = [],
...
getOrgChartJob.prototype._zK = function() { this.zoom(true, true) };
getOrgChartJob.prototype._zL = function() { this.zoom(false, true) };
getOrgChartJob.prototype._zAutoSize = function() { this.zoom(false, false) };
getOrgChartJob.prototype._zy = function(b, a) { this._zh = this._X._u.scrollTop + a[0].touches[0].pageY };
getOrgChartJob.prototype._zb = function(b, a) { this._X._u.scrollTop = this._zh - a[0].touches[0].pageY };
//getOrgChartJob.prototype._zn = function(d, b) {
//    this._X._n = undefined;
//    var a = b[0].wheelDelta ? b[0].wheelDelta / 40 : b[0].detail ? -b[0].detail : 0;
//    if (a) {
//        var e = a > 0;
//        var c = getOrgChartJob.util._aZ(this._X._v, b[0].pageX, b[0].pageY);
//        this.zoom(e, false, c)
//    }
//    return b[0].preventDefault() && false
//};
getOrgChartJob.prototype._zd = function(b, a) {
    var c = this;
    window.setTimeout(function() { c.resize() }, 300)
};
getOrgChartJob.prototype._aQ = function(g, b) {
    this._X._n = undefined;
    if (this._aO.dragStart) {
        ...
        var j = getOrgChartJob.util._5(this._X);
        var k = j[2] / this._aP;
        var c = j[3] / this._aL;
        var a = k > c ? k : c;
...
getOrgChartJob.prototype.zoom = function(c, a, b) {
    ...
    if (c === true) {
        j[2] = j[2] / (getOrgChartJob.SCALE_FACTOR * 1.2);
        j[3] = j[3] / (getOrgChartJob.SCALE_FACTOR * 1.2)
    } else {
        if (c === false) {
            j[2] = j[2] * (getOrgChartJob.SCALE_FACTOR * 1.2);
            j[3] = j[3] * (getOrgChartJob.SCALE_FACTOR * 1.2)
...
getOrgChartJob.prototype._aD = function(c, b) {
...
getOrgChartJob.prototype._za = function(c, b) {
    if (c.className.indexOf("get-disabled") > -1) { return false }
    var a = this;
    clearTimeout(this._zu.timer);
    this._zu.timer = setTimeout(function() {
...
getOrgChartJob.prototype._zm = function(c, b) {
    var a = this;
    clearTimeout(this._zu.timer);
    this._zu.timer = setTimeout(function() {
...
getOrgChartJob.prototype._zi = function(c, b) {
    var a = this;
    clearTimeout(this._zu.timer);
    this._zu.timer = setTimeout(function() {
        a._zu.currentIndex = 0;
...
getOrgChartJob.prototype._aq = function() { if (this._zu.found.length) { this.highlightNode(this._zu.found[this._zu.currentIndex].node.id) } };
getOrgChartJob.prototype.highlightNode = function(c) {
    var a = this;

    function b() {
        var d = a.nodes[c];
        var e = getOrgChartJob.util._5(a._X);
        ...
        a.move([f, g, a.initialViewBoxMatrix[2], a.initialViewBoxMatrix[3]], null, function() {
            ...
            var h = getOrgChartJob.util._3(i);
            var j = h.slice(0);
            j[0] = getOrgChartJob.HIGHLIGHT_SCALE_FACTOR;
            j[3] = getOrgChartJob.HIGHLIGHT_SCALE_FACTOR;
            j[4] = j[4] - ((d.w / 2) * (getOrgChartJob.HIGHLIGHT_SCALE_FACTOR - 1));
            j[5] = j[5] - ((d.h / 2) * (getOrgChartJob.HIGHLIGHT_SCALE_FACTOR - 1));
...
getOrgChartJob.prototype._an = function(a) {};
getOrgChartJob.prototype._l = function() { if ((this._zu.currentIndex < this._zu.found.length - 1) && (this._zu.found.length != 0)) { this._X._aE.className = this._X._aE.className.replace(" get-disabled", "") } else { if (this._X._aE.className.indexOf(" get-disabled") == -1) { this._X._aE.className = this._X._aE.className + " get-disabled" } } if ((this._zu.currentIndex != 0) && (this._zu.found.length != 0)) { this._X._zq.className = this._X._zq.className.replace(" get-disabled", "") } else { if (this._X._zq.className.indexOf(" get-disabled") == -1) { this._X._zq.className = this._X._zq.className + " get-disabled" } } };
getOrgChartJob.prototype._C = function(g) {
    var e = [];
    if (g == "") { return e }
    ...
    for (var b in this.nodes) {
        ...
        for (m in f.data) {
            ...
            var c = -1;
            if (getOrgChartJob.util._ag(f) && getOrgChartJob.util._ag(f.data[m])) {
...
getOrgChartJob.prototype._aT = function(g, a) {
...
getOrgChartJob.prototype._zl = function(a, d, e, c) {
    var b = this._X.getButtonByType(a);
    b.style.display = "block";
    b.setAttribute("transform", "matrix(0.14,0,0,0.14," + d + "," + e + ")");
    b.setAttribute("data-btn-id", c)
};
getOrgChartJob.prototype._aF = function(d, a) { var b = d.getAttribute("data-node-id"); var c = this.nodes[b]; if (!this._R("clickNodeEvent", { node: c })) { return } };
getOrgChartJob.prototype._aR = function(d, b) { b[0].preventDefault(); var c = d.getAttribute("data-btn-id"); var a = d.getAttribute("data-btn-action"); if (a == "del") { this.removeNode(c) } else { if (a == "add") { this.insertNode(c) } else { if (a == "details") { this.showDetailsView(c) } else { if (a == "edit") { this.showEditView(c) } else { if (a == "expColl") { this.expandOrCollapse(c) } } } } } };
getOrgChartJob.prototype.showEditView = function(a) {
    this._S = true;
    this.showDetailsView(a)
};
getOrgChartJob.prototype.expand = function(b, a) { b.collapsed = getOrgChartJob.EXPANDED; if ((b.parent == this._zr) || (b.parent == null)) { this.loadFromJSON(this.nodes, true, a) } else { this.expand(b.parent, a) } };
getOrgChartJob.prototype.expandOrCollapse = function(a) {
    var c = this;
    var b = this.nodes[a];
    this._aN = { id: a, old_x: b.x, old_y: b.y };
    if (b.collapsed == getOrgChartJob.EXPANDED) { b.collapsed = getOrgChartJob.COLLAPSED } else { b.collapsed = getOrgChartJob.EXPANDED } this.loadFromJSON(this.nodes, true, function() { if (b.collapsed == getOrgChartJob.EXPANDED) { c.moveToMostDeepChildForNode(b) } c._R("updatedEvent") })
};
getOrgChartJob.prototype.moveToMostDeepChildForNode = function(b) {
    var c = getOrgChartJob.util._5(this._X);
...
getOrgChartJob.prototype.insertNode = function(d, a, b) {
    var f = this;
    var e = this.nodes[d];
    this._aN = { id: d, old_x: e.x, old_y: e.y };
    if (b == undefined) { b = getOrgChartJob.util._V(this.nodes) }
...
getOrgChartJob.prototype.removeNode = function(b) {
    var e = this;
...
getOrgChartJob.prototype.updateNode = function(b, d, a) {
    var e = this;
    var c = this.nodes[b];
    c.pid = d;
    c.data = a;
...
getOrgChartJob.prototype._h = function(d, f, g, c, a, b) {
    var h = this.config.customize[d] && this.config.customize[d].theme ? getOrgChartJob.themes[this.config.customize[d].theme] : this.theme;
    a = (a == undefined ? getOrgChartJob.NOT_DEFINED : a);
    var e = new getOrgChartJob.node(d, f, g, c, h.size, this.config.photoFields, a, b);
    ...
    this.nodes[d] = e;
    for (label in e.data) { if (!getOrgChartJob.util._s(this._au, label)) { this._au.push(label) } }
    return e
};
getOrgChartJob.prototype.load = function() { var a = this.config.dataSource; if (!a) { return } if (a.constructor && (a.constructor.toString().indexOf("HTML") > -1)) { this.loadFromHTMLTable(a) } else { if (typeof(a) == "string") { this.loadFromXML(a) } else { this.loadFromJSON(a) } } };
getOrgChartJob.prototype.loadFromJSON = function(g, p, a) {
    this._zv = 0;
    this._zf = 0;
    this._zr = new getOrgChartJob.node(-1, null, null, null, 2, 2);
    if (p) {
    ...
    if (this.config.layout == getOrgChartJob.MIXED_HIERARCHY_RIGHT_LINKS) {
        for (var e in this.nodes) {
            var k = this.nodes[e];
            var n = this.nodes[k.pid];
            var j = e;
            if (k.children.length == 0) {
                for (z = n.children.length - 1; z >= 0; z--) {
                    ...
                    if (b.children.length == 0 && b != this.nodes[j] && b.displayPid == null && b.displayChildren == null) {
                        if (this.nodes[j].color == null) { this.nodes[j].color = getOrgChartJob.util._1(this.config.color) } b.color = this.nodes[j].color;
                        b.displayPid = j;
...
getOrgChartJob.prototype.loadFromHTMLTable = function(c) {
    var d = c.rows[0];
    var g = [];
...
getOrgChartJob.prototype.loadFromXML = function(c) {
    var a = this;
    var b = [];
...
getOrgChartJob.prototype.loadFromXMLDocument = function(b) {
    var a = [];
    var c = getOrgChartJob.util._zS(b);
    this._ai = 0;
    this._am(c, null, true, a);
    this.loadFromJSON(a)
};
getOrgChartJob.prototype._am = function(m, l, d, h) {
    var a = this;
    if (m.nodeType == 1) {
...
getOrgChartJob.prototype._y = function(d, b) {
    var a = b[0];
    a.preventDefault();
    var c = new Date().getTime();
    if (b[0].touches.length == 1) {
        var f = getOrgChartJob.util._5(this._X);
    ...
    if (b[0].touches.length == 2) {
        this._aO.dragStart = null;
        this._p = null;
        this._a2 = getOrgChartJob.util._a1(this._X._v, a)
    }
};
getOrgChartJob.prototype._b = function(j, d) {
    var c = d[0];
    c.preventDefault();
    if (d[0].touches.length == 1) {
        ...
        if (this._aO.dragStart) {
            ...
            this._X._t.style.cursor = "move";
            var l = getOrgChartJob.util._5(this._X);
            var m = l[2] / this._aP;
            var f = l[3] / this._aL;
            var a = m > f ? m : f;
    ...
    if (d[0].touches.length == 2) {
        var b = getOrgChartJob.util._a1(this._X._v, d[0]);
        var k = 1 + ((b - this._a2) / (this._a2 / 100)) / 100;
        var i = getOrgChartJob.util._a3(this._X._v, d[0]);
        this.zoom(k, null, i);
        this._a2 = b
    }
};
getOrgChartJob.prototype._g = function(d, b) {
    var a = b[0];
    a.preventDefault();
    var c = new Date().getTime()
};
getOrgChartJob.prototype._aH = function(i, c) {
    var b = c[0];
    b.preventDefault();
    this._aF(i, c);
    this._aT(i, c);
...
getOrgChartJob.prototype._aY = function(c, b) {
    var a = b[0];
    a.preventDefault()
};
getOrgChartJob.prototype._aB = function(c, b) {
    var a = b[0];
    a.preventDefault();
    if (a.touches && a.touches.length && a.touches.length == 0) { this._aV = null }
};
getOrgChartJob.themes.OdooTheme = {
    size: [650, 290],
    toolbarHeight: 46,
    textPoints: [
    ...
    defs: '<filter id="f3" x="0" y="0" width="200%" height="200%"><feOffset result="offOut" in="SourceAlpha" dx="5" dy="5" /><feGaussianBlur result="blurOut" in="offOut" stdDeviation="5" /><feBlend in="SourceGraphic" in2="blurOut" mode="normal" /></filter>',
    box: '<rect x="0" y="0" height="290" width="650" rx="10" ry="10" class="OdooTheme-box" filter="url(#f3)"  />',
