...
class account_analytic_account(models.Model):
    ...
    booking_order_line_ids = fields.Many2many('booking.order.line',string='Booking Order Line', domain="[('state','=','draft'),('order_id','!=',False)]", copy=False)
    ...
    def validate_booking_order_line(self):
        ...
        for line_id in self.booking_order_line_ids:
            ...
            if line_id.order_id:
                ...
                out_stock_picking_id = picking_obj.search([('min_date','=',line_id.start_date), ('state','=','draft'), ('picking_type_id','=', outgoing_type_id.id)], limit=1)
                if not out_stock_picking_id:
                        out_stock_picking_id = line_id.order_id.create_stock_picking('do_out')
                in_stock_picking_id = picking_obj.search([('min_date','=',line_id.end_date),('state','=','draft'), ('picking_type_id','=', incoming_type_id.id)], limit=1)
