...
class crm_team_popup(models.TransientModel):
    ...
    def print_crm_team(self):
        ...
        achievement = []
        result = {}
        if self.env.context.get('active_id', False) and self.env.context.get('active_model', False) == 'crm.team':
            crm_team = self.env['crm.team'].browse(self.env.context.get('active_id', False))
            if self.input_method == 'mothly':
                for i in range(1, 13):
                    ...
                    if sales:
                        sale_order_ids = sales.filtered(lambda record: record.user_id in crm_team.member_ids and record.user_id.branch_id in self.branch_id)
                        if sale_order_ids:
                    ...
                    pos = self.env['pos.order'].search(args).filtered(lambda record:
                         ...
                         self.change_datetime(record.date_order).month == i)
                    if pos:
                        pos_order_ids = pos.filtered(lambda record: record.user_id in crm_team.member_ids and record.user_id.branch_id in self.branch_id)
                        if pos_order_ids:
                ...
                for i in range(1, count_day + 1):
                    ...
                    if pos:
                        pos_order_ids = pos.filtered(lambda
                                                         record: record.user_id in crm_team.member_ids and record.user_id.branch_id in self.branch_id)
                        if pos_order_ids:
                            for order in pos_order_ids:
                                achievement_amount += order.amount_total
                        achievement.append(achievement_amount)
