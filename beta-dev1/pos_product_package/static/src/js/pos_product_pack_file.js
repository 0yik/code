...
odoo.define('pos_product_package.pos_product_package', function(require) {
	"use strict";
	// alert('pos_product_packagepos_product_package')
	...
	models.load_models([{
		...
		condition: function(self) {
			return true; },
		fields: ['id', 'product_name', 'product_quantity', 'uom_id', 'price', 'name', 'pos_categ_id'],
		domain: function(self) {
			return []; },
		loaded: function(self, result) {
					...
					for (var j = 0; j < (pack_product[i].wk_product_pack).length; j++) {
						for (var k = 0; k < wk_pack_products.length; k++) {
							if (wk_pack_products[k].id == pack_product[i].wk_product_pack[j]) {
								console.log('wk_pack_products[k]  ',wk_pack_products[k])
								var product_val = { 'display_name': wk_pack_products[k].name, 'uom_id': wk_pack_products[k].uom_id, 'price': wk_pack_products[k].price, 'pos_categ_id': wk_pack_products[k].pos_categ_id};
