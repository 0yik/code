odoo.define('honos_email_subscriber.email_subscriber', function (require) {
	'use strict';
...
ajax.jsonRpc('/web/dataset/call-public', 'call', {
    ...
    'args': ['honos_email_subscriber.honos_email_subscriber_popup', base.get_context()]
}).done(function (data) {
...
var Email_popup = Widget.extend({
	template: 'honos_email_subscriber.honos_email_subscriber_popup',
    events: {
        'click .close': 'close',
    },
    start: function () {
    ...
    on_click_subscribe: function () {
        ...
        ajax.jsonRpc('/website_mass_mailing/subscribe', 'call', {
        	...
        	self.setEmailCookie($email.length ? $email.val() : 'public@honos');
    ...
    setEmailCookie: function (email) {
    	var email_val = email.length ? email : 'public@honos'
    	var date = new Date();   
    	date.setTime(date.getTime()+(30*24*60*60*1000)); 
    	var expires = "expires=" + date.toGMTString();
    	document.cookie = "subscriber_popup-"+session_uid+"=" + email_val + ";" + expires +"; path=/";
    },
