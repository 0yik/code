...
class honosSignUp(AuthSignupHome):

    def get_auth_signup_config(self):
        res = super(honosSignUp,self).get_auth_signup_config()
        IrConfigParam = request.env['ir.config_parameter']
        attachment_id = IrConfigParam.sudo().get_param('honos_signup.signup_bg_image_id',False)
