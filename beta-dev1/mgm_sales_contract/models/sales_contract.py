        ...
        if self.pricelist_id:
            for line in self.line_ids:
                line.price_unit = self.pricelist_id.price_get(line.product_id.id,
                                                              line.product_uom_qty, self.partner_id.id)[
...
class sale_contract_line(models.Model):
    ...
    currency_id = fields.Many2one("res.currency", related='requisition_id.currency_id', string="Currency", readonly=True)
    ...
    def _compute_sub_total(self):
        for line in self:
            if line.requisition_id:
                apply_flag = False
                total_undiscount_price = line.product_uom_qty * line.price_unit
                if line.requisition_id.pricelist_id:
                    pricelist = line.requisition_id.pricelist_id
                    for pricelist_item in pricelist.item_ids:
                        apply_on = pricelist_item.applied_on

                        if apply_on == '3_global':
                            apply_flag = True
                        elif apply_on == '2_product_category':
                            if line.product_id.categ_id.id  == pricelist_item.categ_id.id:
                                apply_flag = True
                        elif apply_on == '1_product':
                            if line.product_id.product_tmpl_id.id == pricelist_item.product_tmpl_id.id:
                                apply_flag = True
                        else:
                            if line.product_id.id == pricelist_item.product_id.id:
                                apply_flag = True
                        if apply_flag:
                            if pricelist_item.compute_price:
                                compute_price = pricelist_item.compute_price
                                if compute_price:
                                    if compute_price == 'fixed':
                                        if pricelist_item.fixed_price:
                                            line.sub_total = total_undiscount_price - pricelist_item.fixed_price
                                            line.sub_total = line.sub_total < 0 and 0 or line.sub_total
                                            break
                                    if compute_price == 'percentage':
                                        if pricelist_item.percent_price:
                                            line.sub_total = total_undiscount_price - pricelist_item.percent_price/100 * total_undiscount_price
                                            line.sub_total = line.sub_total < 0 and 0 or line.sub_total
                                            break
