...
class PosOrder(models.Model):
    ...
    def reduce_bom_stock(self, order_line):
        def check_bom(bom, qty):
            ...
            return
        bom_obj = self.env['mrp.bom']
        if self._context.get('staff_meal',False):
            pos_order_id = order_line
            pos_config_id = order_line.session_id.config_id
            for line in pos_order_id.lines:
                bom = bom_obj.search([('product_id','=',line.product_id.id)])
                check_bom(bom, line.qty)
        elif self._context.get('by_delivery',False):
            sale_order = order_line
            pos_config_id = sale_order.session_id.config_id
            for line in sale_order.order_line:
                bom = bom_obj.search([('product_id','=',line.product_id.id)])
                check_bom(bom, line.product_uom_qty)
        else:
    ...
    def paid_order_open(self,id):
        ...
        order.with_context({'staff_meal':True}).reduce_bom_stock(order)
        order.state = 'paid'
