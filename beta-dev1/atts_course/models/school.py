...
class SubjectSubject(models.Model):
    ...
    industry_level = fields.Many2many('course.industry', 'course_industry_rel', 'industry_id', 'course_id', 'Industry Level')
    course_level = fields.Many2many('course.level', 'course_level_rel', 'level_id', 'course_id', 'Course Level')
    course_fee = fields.Float('Course Fee', digits=(4,2))
    duration_of_course = fields.Char('Duration of course')
    cert_retention_period = fields.Integer('Cert Retention Period')
    cert_expiry_period = fields.Integer('Cert Expiry Period')
    module1 = fields.Char('Module 1')
    module2 = fields.Char('Module 2')
    module3 = fields.Char('Module 3')
    module4 = fields.Char('Module 4')
    module5 = fields.Char('Module 5')
    module6 = fields.Char('Module 6')
...
class CourseIndustry(models.Model):
    ''' Defining Course Industry '''
    _name = 'course.industry'
    _description = "Industry Level"

    name = fields.Char("Name")


class CourseLevel(models.Model):
    ''' Defining Course Level '''
    _name = 'course.level'
...
class StudentStudent(models.Model):
    ...
    def create(self, vals):
        ...
        #                      _('''PID not valid so record will not be saved.'''))
        ...
        # if res.state == 'draft':
        #     admission_group = self.env.ref('atts_course.group_is_admission')
        #     new_grp_list = [admission_group.id, emp_grp.id]
        #     res.user_id.write({'groups_id': [(6, 0, new_grp_list)]})
        # elif res.state == 'done':
        #     done_student = self.env.ref('atts_course.group_school_student')
    ...
    pid = fields.Char('Student ID', default=lambda obj:
    ...
    dermatological = fields.Boolean('0Dermatological')
...
#        if res and res.is_school_teacher:
        if vals.get('school') and res.is_school_teacher:
            ...
            if res and user:
                ...
                teacher_group = self.env.ref('atts_course.group_school_teacher')
                emp_group = self.env.ref('base.group_user')
            ...
            if rec.school and rec.is_school_teacher and not rec.user_id:
                ...
                if res and user:
                    rec.write({'address_home_id': user.partner_id.id,
                               'user_id': user and user.id or False})
                    teacher_grp = self.env.ref('atts_course.group_school_teacher')
                    emp_group = self.env.ref('base.group_user')
                    user.write({'groups_id': [(6, 0, [emp_group.id,
                                                      teacher_grp.id])]})
        # Assign email
...
class ResUsers(models.Model):
    ...
    student_id = fields.One2many('student.student', 'user_id', 'Student ID')
    ...
    def create(self, vals):
        ...
        value = {
            'name': res.name,
            'user_id': res.id,
            'street': res.partner_id.street,
            'street2': res.partner_id.street2,
            'zip': res.partner_id.zip,
            'city': res.partner_id.city,
            'state_id': res.partner_id.state_id,
            'country_id': res.partner_id.country_id,
            'email': res.partner_id.email,
            'color': res.partner_id.color,
            'website': res.partner_id.website,
            'comment': res.partner_id.comment,
            'phone': res.partner_id.phone,
            'mobile': res.partner_id.mobile,
            'company_name': res.partner_id.company_name,
            'fax_no': res.partner_id.fax_no,
            'uen_no_company_number': res.partner_id.uen_no_company_number,
            'company_address': res.partner_id.company_address,
            'mail_address': res.partner_id.company_address,
            'certi_mailing_add': res.partner_id.company_address,
        }
        self.env['student.student'].create(value)
        return res
