...
class WooModelBinder(WooBinder):
    ...
    def to_backend(self, record_id, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                return binding.quickbook_id
            else:
                return None
        if not record:
            record = self.model.browse(record_id)
        assert record
        return record.quickbook_id
    ...
    def bind(self, external_id, binding_id):
        ...
        # avoid to trigger the export when we modify the `quickbook_id`
