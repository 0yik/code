...
class WooBaseExporter(Exporter):
    ...
    def _delay_import(self):
        ...
        assert self.quickbook_id
        import_record.delay(self.session, self.model._name,
                            self.backend_record.id, self.quickbook_id,
                            force=True)

    def _should_import(self):
        """ Before the export, compare the update date
        in Quickbook and the last sync date in OpenERP,
        if the former is more recent, schedule an import
        to not miss changes done in Quickbook.
        """
        assert self.binding_record
        if not self.quickbook_id:
            return False
        sync = self.binding_record.sync_date
        if not sync:
            return True
        record = self.backend_adapter.read(self.quickbook_id,
    ...
    def run(self, binding_id, *args, **kwargs):
        ...
        self.quickbook_id = self.binder.to_backend(self.binding_id)
        try:
        ...
        self.binder.bind(self.quickbook_id, self.binding_id)
    ...
    def _after_export(self):
        """ Can do several actions after exporting a record on Quickbook """
        pass


class WooExporter(WooBaseExporter):

    """ A common flow for the exports to Quickbook """
    ...
    def _update(self, data):
        """ Update an Quickbook record """
        assert self.quickbook_id
        # special check on data before export
        self._validate_update_data(data)
        self.backend_adapter.write(self.quickbook_id, data)
    ...
    def _run(self, fields=None):
        ...
        if self.quickbook_id:
            ...
            if not record:
                return _('Nothing to export.')
            self.quickbook_id = self._create(record)
        return _('Record exported with ID %s on Quickbook.') % self.quickbook_id


@job(default_channel='root.woo')
@related_action(action=unwrap_binding)
def export_record(session, model_name, binding_id, fields=None):
    """ Export a record on Quickbook """
