odoo.define('stock_inventory_request_extended.cart_inventory', function (require) {
//     "use strict";
    ...
    var sale_cart_link = $('ul#top_menu li a[href$="/inventory/cart"]');
    ...
    sale_cart_link.popover({
        ...
        sale_cart_link_counter = setTimeout(function(){
            ...
            {
                $.get("/inventory/cart", {'type': 'popover'})
                    .then(function (data) {
...
odoo.define('stock_inventory_request_extended.stock_inventory_request_extended', function (require) {
    "use strict";
    ...
    $('.oe_website_sale').each(function () {
        ...
        $(oe_website_sale).on("change", 'input[name="add_qty"]', function (event) {
            ...
            if ($("#product_detail").length) {
                ...
                ajax.jsonRpc("/inventory/get_unit_price", 'call', {'product_ids': product_ids,'add_qty': parseInt(qty)})
                .then(function (data) {
                    _.each(product_dom, function (prod) {
        ...
        $(oe_website_sale).on("change", ".oe_cart_inventory input.js_quantity[data-product-id]", function () {
          var $input = $(this);
          ...
          clickwatch(function(){
            ...
            ajax.jsonRpc("/inventory/cart/update_json", 'call', {
                ...
                $(".js_cart_lines").first().before(data['stock_inventory_request_extended.cart_lines']).end().remove();

                if (data.warning) {
                    var cart_alert = $('.oe_cart_inventory').parent().find('#data_warning');
                    if (cart_alert.length === 0) {
                        $('.oe_cart_inventory').prepend('<div class="alert alert-danger alert-dismissable" role="alert" id="data_warning">'+
        ...
        $(oe_website_sale).on("click", ".oe_cart_inventory a.js_add_suggested_products", function () {
            $(this).prev('input').val(1).trigger('change');
        });

        // hack to add and remove from cart with json
        $(oe_website_sale).on('click', 'a.js_add_cart_json_inventory', function (ev) {
            ev.preventDefault();
        ...
        $('form.js_add_cart_json_inventory label', oe_website_sale).on('mouseup touchend', function () {
            var $label = $(this);
        ...
        $('.oe_cart_inventory').on('click', '.js_change_shipping', function() {
        ...
        $('.oe_cart_inventory').on('click', '.js_edit_address', function() {
            $(this).parent('div.one_kanban').find('form.hide').attr('action', '/sale/address').submit();
        });
        $('.oe_cart_inventory').on('click', '.js_delete_product', function(e) {
            e.preventDefault();
