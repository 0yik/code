    ...
    def _lines(self, line_id=None):
        ...
        for account in sorted_accounts:
            ...
            for count in xrange(len(context['periods'])):
                ...
                debit_credit.extend([self._format(debit), self._format(credit)])
                debit_credit_total.update({count: [debit_credit_total.get(count)[0] + debit, debit_credit_total.get(count)[1] + credit]})
            lines.append({
                'id': account.id,
                'type': 'account_id',
