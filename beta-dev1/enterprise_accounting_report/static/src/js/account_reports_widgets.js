        ...
        return this.context_model.call('add_line', [[parseInt(context_id, 10)], parseInt(active_id, 10)]).then(function () { // First add the line to the model
            ...
            if (!isLoaded) { // If the lines have not yet been loaded
                ...
                new Model('account.report.context.common').call('get_full_report_name_by_report_name', [report_name]).then(function (result) {
                    ...
                    // Updating context
                    if (self.context.aging_due_filter_cmp) {
                        self.odoo_context.aging_due_filter_cmp = true;
                    }
                    if (self.context.aging_filter_cmp) {
                        self.odoo_context.aging_filter_cmp = true;
                    }
                    if (self.context.filter_original_currency) {
                        self.odoo_context.filter_original_currency = true;
                    }
                    if (self.context.filter_local_currency) {
                        self.odoo_context.filter_local_currency = true;
                    }
