odoo.define('enterprise_accounting_report.account_report_generic', function (require) {
'use strict';
...
var ReportWidget = require('enterprise_accounting_report.ReportWidget');
...
var account_report_generic = Widget.extend(ControlPanelMixin, {
    ...
    render_searchview_buttons: function() {
        ...
        var $dateFilter = this.$searchview_buttons.siblings('.o_enterprise_accounting_report_date-filter');
        var $dateFilterCmp = this.$searchview_buttons.siblings('.o_enterprise_accounting_report_date-filter-cmp');
        var $useCustomDates = $dateFilter.find('.o_enterprise_accounting_report_use-custom');
        var $CustomDates = $dateFilter.find('.o_enterprise_accounting_report_custom-dates');
        $useCustomDates.bind('click', function () {self.toggle_filter($useCustomDates, $CustomDates);});
        var $usePreviousPeriod = $dateFilterCmp.find('.o_enterprise_accounting_report_use-previous-period');
        var $previousPeriod = $dateFilterCmp.find('.o_enterprise_accounting_report_previous-period');
        $usePreviousPeriod.bind('click', function () {self.toggle_filter($usePreviousPeriod, $previousPeriod);});
        var $useSameLastYear = $dateFilterCmp.find('.o_enterprise_accounting_report_use-same-last-year');
        var $SameLastYear = $dateFilterCmp.find('.o_enterprise_accounting_report_same-last-year');
        $useSameLastYear.bind('click', function () {self.toggle_filter($useSameLastYear, $SameLastYear);});
        var $useCustomCmp = $dateFilterCmp.find('.o_enterprise_accounting_report_use-custom-cmp');
        var $CustomCmp = $dateFilterCmp.find('.o_enterprise_accounting_report_custom-cmp');
        $useCustomCmp.bind('click', function () {self.toggle_filter($useCustomCmp, $CustomCmp);});
        this.$searchview_buttons.find('.o_enterprise_accounting_report_one-filter').bind('click', function (event) {
            self.onChangeDateFilter(event); // First trigger the onchange
            var error = false;
            $('.o_enterprise_accounting_report_datetimepicker input').each(function () { // Parse all the values of the date pickers
                if (error) {return;}
                if ($(this).val() === ""){
        ...
        this.$searchview_buttons.find('.o_enterprise_accounting_report_one-filter-cmp').bind('click', function (event) { // Same for the comparison filter
            self.onChangeCmpDateFilter(event);
            $('.o_enterprise_accounting_report_datetimepicker input').each(function () {
                $(this).val(formats.parse_value($(this).val(), {type: 'date'}));
            });
        ...
        this.$searchview_buttons.find('.o_enterprise_accounting_report_one-filter-bool').bind('click', function (event) { // Same for the boolean filters
            var report_context = {};
        ...
        if (this.report_context.multi_company) { // Same for th ecompany filter
            this.$searchview_buttons.find('.o_enterprise_accounting_report_one-company').bind('click', function (event) {
                var report_context = {};
        ...
        if (this.report_context.journal_ids) { // Same for the journal
            this.$searchview_buttons.find('.o_enterprise_accounting_report_one-journal').bind('click', function (event) {
                var report_context = {};
                var value = $(event.target).parents('li').data('value');
        ...
        if (this.report_context.account_type) { // Same for the account types
            this.$searchview_buttons.find('.o_enterprise_accounting_report_one-account_type').bind('click', function (event) {
                var value = $(event.target).parents('li').data('value');
                self.restart({'account_type': value});
            });
        }
        if (this.report_type.analytic && this.report_context.analytic) { // Same for the tags filter
            this.$searchview_buttons.find(".o_enterprise_accounting_report_analytic_account_auto_complete").select2();
            var selection = [];
            ...
            this.$searchview_buttons.find('.o_enterprise_accounting_report_analytic_account_auto_complete').data().select2.updateSelection(selection);
            this.$searchview_buttons.find(".o_enterprise_accounting_report_analytic_tag_auto_complete").select2();
            selection = [];
            var i;
            ...
            this.$searchview_buttons.find('.o_enterprise_accounting_report_analytic_tag_auto_complete').data().select2.updateSelection(selection);
            this.$searchview_buttons.find('.o_enterprise_accounting_report_add_analytic_account_tag').bind('click', function (event) {
                var report_context = {};
                var value = self.$searchview_buttons.find(".o_enterprise_accounting_report_analytic_account_auto_complete").select2("val");
                report_context.analytic_account_ids = value;
                value = self.$searchview_buttons.find(".o_enterprise_accounting_report_analytic_tag_auto_complete").select2("val");
        ...
        var $datetimepickers = this.$searchview_buttons.find('.o_enterprise_accounting_report_datetimepicker');
    ...
    onChangeCmpDateFilter: function(event, fromDateFilter) {
        ...
        if (filter_cmp === 'previous_period' || filter_cmp === 'same_last_year') {
            ...
            if (!no_date_range) {
                this.$searchview_buttons.find("input[name='date_from_cmp']").parents('.o_enterprise_accounting_report_datetimepicker').data("DateTimePicker").setValue(moment(dtFrom));
            }
            this.$searchview_buttons.find("input[name='date_to_cmp']").parents('.o_enterprise_accounting_report_datetimepicker').data("DateTimePicker").setValue(moment(dtTo));
        }
    },
    onChangeDateFilter: function(event) {
        ...
        switch($(event.target).parents('li').data('value')) {
            case 'today':
                dt = new Date();
                self.$searchview_buttons.find("input[name='date_to']").parents('.o_enterprise_accounting_report_datetimepicker').data("DateTimePicker").setValue(moment(dt));
                break;
            case 'last_month':
                dt = new Date();
                ...
                if (!no_date_range) {
                    ...
                    self.$searchview_buttons.find("input[name='date_from']").parents('.o_enterprise_accounting_report_datetimepicker').data("DateTimePicker").setValue(moment(dt));
                }
                break;
            case 'last_quarter':
                dt = new Date();
            ...
            case 'last_year':
                ...
                $("input[name='date_to']").parents('.o_enterprise_accounting_report_datetimepicker').data("DateTimePicker").setValue(moment(dt));
                if (!no_date_range) {
            ...
            case 'this_quarter':
                dt = new moment();
                self.$searchview_buttons.find("input[name='date_to']").parents('.o_enterprise_accounting_report_datetimepicker').data("DateTimePicker").setValue(dt.endOf('quarter'));
                if (!no_date_range) {
                    self.$searchview_buttons.find("input[name='date_from']").parents('.o_enterprise_accounting_report_datetimepicker').data("DateTimePicker").setValue(dt.startOf('quarter'));
                }
                break;
            case 'this_year':
                if (today.getMonth() + 1 < self.fy.fiscalyear_last_month || (today.getMonth() + 1 === self.fy.fiscalyear_last_month && today.getDate() <= self.fy.fiscalyear_last_day)) {
