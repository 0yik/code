...
import pytz
import os
from odoo.addons.modifier_online_school_enrollment.controllers.main import AdminRegister
import logging
    ...
    def admission_register_create(self, **post):
        ...
        if post:
            ...
            if not already_student_id:
                ...
                if student_register_id and student_register_id.standard_id and student_register_id.standard_id.is_from_course:
                    payslip_id = request.env['student.payslip'].sudo().search([('student_id','=',student_register_id.id)], limit=1)
                    line_list = []
                    if payslip_id:
                        for line in payslip_id.line_ids:
                            line.sudo().unlink()
                        payslip_id.fees_structure_id = student_register_id.standard_id.application_fee_structure_id.id
                        for fee_structure_line in  student_register_id.standard_id.application_fee_structure_id.line_ids:
                            vals = {
                                ...
                                'slip_id': payslip_id and payslip_id.id or False
                            }
                            request.env['student.payslip.line'].sudo().create(vals)

            else:
                # already_student_id = request.env['student.student'].sudo().create(vals)
                if already_student_id and already_student_id.standard_id and already_student_id.standard_id.is_from_course:
                    payslip_id = request.env['student.payslip'].sudo().search([('student_id','=',already_student_id.id)], limit=1)
                    if payslip_id:
                        payslip_id.fees_structure_id = already_student_id.standard_id.application_fee_structure_id.id
            ...
            if post.has_key('payment_option') and post.get('payment_option',False) != 'manually':
                if post.get('school_ids',False) and int(post.get('school_ids',False)) and (already_student_id or student_register_id):
                    if post.get('standard_ids',False) and int(post.get('standard_ids',False)):
                        self.send_student_mail(already_student_id or student_register_id)
                        standard_ids = post.get('standard_ids',False) and int(post.get('standard_ids',False))
                        standard_browse_ids = request.env['school.standard'].sudo().browse(standard_ids)
                        amount = 0
                        if standard_browse_ids.application_fee_structure_id:
                            env = request.env
                            user = env.user.sudo()
                            currency_id = user.company_id.currency_id.id
                            country_id = user.company_id.country_id.id
                            for line in standard_browse_ids.application_fee_structure_id.line_ids:
                                if line.type == 'application_fee':
                                    values.update({'amount':line.amount,'name': line.name})
                            reference = (already_student_id and already_student_id.name +'-'+ standard_browse_ids.standard_id.name + '-' + str(already_student_id.nric or '') + '-' + str(already_student_id.id)) or (student_register_id.name +'-'+ standard_browse_ids.standard_id.name + '-' +  str(student_register_id.nric or '') + '-' + str(student_register_id.id))
                ...
                self.send_student_mail(already_student_id or student_register_id)
                if already_student_id or student_register_id:
