odoo.define('mgm_sales_dashboard.sales_dashboard', function (require) {
"use strict";
...
var SalesTeamDashboardViewmodifier = KanbanView.extend({
    display_name: _lt('Dashboard modifier'),
    icon: 'fa-dashboard',
    ...
    fetch_data: function() {
        return new Model('crm.lead').call('retrieve_sales_dashboard', []);
    },
    ...
    render: function() {
        ...
        return this.fetch_data().then(function(result){
            ...
            new Model("crm.team").call("get_sale_requisition_dashboard").then(function (result_data) {
                var sales_dashboard = QWeb.render('MgmSalesDashboard', {
                    widget: self,
                    show_demo: self.show_demo,
                    values: result,
                    sales_requisition: result_data,
                });
               /*other template(SalesDashboardTitle) render
                var sales_dashboard_title = QWeb.render('SalesDashboardTitle', {
                    widget: self,
                    show_demo: self.show_demo,
                    values: result,
                    sales_requisition: result_data,
                });
                //$(sales_dashboard_title).prependTo(self.$el);
                */
                super_render.call(self);
                
                $("<div class='col-xs-12' style='font-size: 18px;'>Sales to Invoice</div>").prependTo(self.$el);
                $(sales_dashboard).appendTo(self.$el);
            });
        });
    },
...
core.view_registry.add('sales_team_dashboard_modifier', SalesTeamDashboardViewmodifier);

return SalesTeamDashboardViewmodifier;
