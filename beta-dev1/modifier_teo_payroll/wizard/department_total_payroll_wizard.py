...
class dpt_total_payroll_wizard(models.TransientModel):

    _name = 'dpt.total.payroll.wizard'
    ...
    department_ids = fields.Many2many('hr.department', 'hr_department_dpt_total_rel', 'dpt_total_id', 'employee_id', 'Employee Name')
...
#                raise ValidationError(_('You must be configure Contact no or email for %s employee.' % (employee.name)))
        ...
        if not payslip_ids.ids:
            raise ValidationError(_('There is no payslip details available between selected date %s and %s') % (date_from, date_to))
        ...
        return self.env['report'].get_action(self, 'teo_payroll.dpt_total_payroll_report_temp' , data = datas)
