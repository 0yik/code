...
class ResDisassemble(models.Model):
    ...
    @api.onchange('stock_production_prod')
    def onchange_stock_production_prod_id(self):
        res = {}
        stock_lot = {}
        if self.stock_production_prod:
            for line in self.stock_production_prod.material_ids:
                stock_lot.update({line.product_id.id: line.stock_lot.id})
            for material_line in self.material_id:
                if stock_lot.get(material_line.product_id.id):
                    material_line.stock_lot = stock_lot.get(material_line.product_id.id)
        return res
    ...
    def compute_product_qty(self):
        quant_ids = self.env['stock.quant'].sudo().search([('product_id','=',self.product_id.id),('location_id','=',self.location_src_id.id)])
        return sum([quant.qty for quant in quant_ids])

    @api.multi
    def action_disassemble(self):
        available_qty = self.compute_product_qty()
