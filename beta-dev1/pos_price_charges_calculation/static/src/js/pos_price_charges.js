odoo.define('pos_price_charges_calculation.pos_price_charges_calculation', function (require) {
"use strict";
    ...
    models.PosModel = models.PosModel.extend({
        process_complimentary_all: function(lines, order){
                    ...
                    if(!pin.trim()){
                        ...
                        if(this.pos.check_pin_number(pin)){
                            for(var i=0; i < lines.length; i++){
                                var line=lines[i]
                                if($('.all-free-button').hasClass('all-free-on')){
                                    line.set_complimentary(false)
                                }
                                else{
                                    line.set_complimentary(true)
                                    line.set_service_charge_value(0)
                                }
                            }
    ...
    var complimentaryOrderButton = screens.ActionButtonWidget.extend({
        ...
        button_click: function() {
            ...
            if(order.all_free && !order.service_charge){
                alert('When All Free is set, Complimentary can not be enabled.');
            }
            else{
                if(!line){
    ...
    var AllFreeOrderButton = screens.ActionButtonWidget.extend({
        ...
        button_click: function() {
            ...
            this.pos.process_complimentary_all(lines,order);
    ...
    return {
        complimentaryOrderButton : complimentaryOrderButton,
