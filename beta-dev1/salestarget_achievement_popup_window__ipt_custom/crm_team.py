...
class ResUsers(models.Model):
    ...
    def get_meeting_ids(self):
        for rc in self:
            ...
            if kq:
                rc.meeting_log_ids = kq
                #self.env['calendar.event'].browse(kq)
    ...
    meeting_log_ids = fields.Many2many('calendar.event', string='Meeting Log', compute='get_meeting_ids')
...
class CalendarEvent(models.Model):
    ...
    state = fields.Selection([('draft', 'Unconfirmed'), ('open', 'Confirmed'), ('done', 'Done')], string='Status', readonly=True, track_visibility='onchang', default='draft')
