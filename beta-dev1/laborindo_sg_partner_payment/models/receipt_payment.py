...
class ReceiptPayment(models.Model):
    ...
    state = fields.Selection(
        [('draft', 'Draft'), ('cancel', 'Cancelled'), ('in_progress', 'In Progress'), ('posted', 'Posted')],
        default='draft', copy=False, track_visibility='onchange', string='Status')
    type = fields.Selection([('customer', 'Customer'), ('supplier', 'Supplier')], default='customer', required=True,
                            string='Type')
    ...
    # exchange_currency_id = fields.Many2one('res.currency', required=True, default=lambda self: self.env.user.company_id.currency_id.id, string='Exchange Currency')
    ...
    date = fields.Date('Date', copy=False, required=True, help='Effective date for accounting entries',
    ...
    # payment_count = fields.Integer(compute='_get_payment_count', copy=False, string='Payments Count')
    payment_difference_handling = fields.Selection([('without_writeoff', 'Keep Open'),
                                                    ('reconcile', 'Reconcile Payment Balance'), ],
                                                   'Payment Difference',
                                                   #                                         required=True,
    ...
    comment = fields.Char('Counterpart Comment', readonly=True, states={'draft': [('readonly', False)]},
                          default='Conterpart')
    ...
    def create(self, vals):
        ...
        # vals.update(self.onchange_currency_rate(vals['currency_id'], vals['exchange_currency_id'])['value'])
    ...
    def load_data(self):
        ...
        if self.partner_id:
            ...
            #     account_ids = self.env['account.account'].search([('internal_type', 'in', ['receivable','payable'])]).ids
            ...
            # self.env.cr.execute('''
            # SELECT id as move_line_id, date, date_maturity, invoice_id, account_id, payment_id, currency_id as move_currency_id, amount_residual, amount_residual as amount_unreconciled,
            # amount_residual_currency as amount_unreconciled_currency, amount_currency as original_amount_currency, balance
            # FROM account_move_line WHERE partner_id = %s and account_id in %s and company_id=%s and amount_residual != 0 and reconciled != True
            # ''' % (self.partner_id.id, account_ids, self.company_id.id))
            ...
            for result in self.env.cr.dictfetchall():
                if result['balance'] != 0:
                    ...
                    result['amount_unreconciled'] = abs(
                        result['amount_unreconciled']) if base_currency_id == currency_id else base_currency_id.compute(
                        abs(result['amount_unreconciled']), currency_id, False)
                    result['amount_unreconciled_currency'] = abs(result['amount_unreconciled_currency']) if result['amount_unreconciled_currency'] else currency_id.compute(abs(result['amount_unreconciled']), base_currency_id, False)
                    result['original_amount_currency'] = abs(result['original_amount_currency'])
                    invoice_id = self.env['account.invoice'].browse(result['invoice_id'])

                    if invoice_id.type in ['out_invoice',
                                           'in_refund'] and invoice_id.state == 'open':
                        # if result['invoice_id'] == 'out_invoice' or result['invoice_id'] == 'in_refund' and result['debit_note'] == False and result['state'] == 'open':
                        original_amount = abs(
                            result['balance']) if base_currency_id == currency_id else base_currency_id.compute(
                            abs(result['balance']), currency_id, False)
                        residual_amount = abs(
                            result['amount_unreconciled_currency']) if base_currency_id == currency_id else base_currency_id.compute(
                            abs(result['amount_unreconciled_currency']), currency_id, False)
                        debit_amount += residual_amount
                        result['original_amount'] = original_amount
                        if not result['original_amount_currency']:
                            result['original_amount_currency'] = currency_id.compute(abs(result['original_amount']),
                                                                                     base_currency_id, False)
                        line_dr_lst.append((0, 0, result))

                    elif invoice_id.type in ['out_refund',
                                             'in_invoice'] and invoice_id.state == 'open':
            ...
            if self.type == 'customer':
                ...
                for dr_lst in line_dr_lst:
                    dr_index += 1
                    if credit_amount > 0:
                        amt = line_dr_lst[dr_index][2]['amount_unreconciled']
                        if amt > credit_amount:
                            ...
                            debit_amount += amt
                            reconcile = True
                        line_dr_lst[dr_index][2].update({'amount': amt, 'reconcile': reconcile})
                debit_amount -= self.amount
                for cr_lst in line_cr_lst:
                    cr_index += 1
                    if debit_amount > 0:
                        amt = line_cr_lst[cr_index][2]['amount_unreconciled']
                        if amt > debit_amount:
                            amt = debit_amount
                            debit_amount -= amt
                            reconcile = False
                            ...
                            debit_amount -= amt
                            reconcile = True
                        line_cr_lst[cr_index][2].update({'amount': amt, 'reconcile': reconcile})
            else:
                debit_amount += self.amount
                credit_amount = 0.0
                for cr_lst in line_cr_lst:
    ...
    #     def get_account_move_line(self,move_id):
    #         move_line_list=[]
    #         difference_amount = self.diff_amount
    ...
    #         if payment_difference_handling == 'reconcile':
    #             reciept_account = self.writeoff_acc_id
    #             debit = 0.0
    #             credit = 0.0
    #             move_line_dict = {
    #                               'name' : 'Counter Part',
    ...
    #                 move_line_dict['account_id'] = reciept_account.id
    #                 move_line_dict['credit'] = float("{0:.2f}".format(difference_amount))
    #                 move_line_list.append(move_line_dict.copy())
    #                 move_line_dict['debit'] =  float("{0:.2f}".format(difference_amount))
    #                 move_line_dict['credit'] = 0.0
    #                 move_line_dict['account_id'] = partner_account.id
    #                 move_line_list.append(move_line_dict.copy())
    #         return move_line_list
        ...
        for record in self:
            ...
            if record.type == 'customer':
                for dr_line in record.line_dr_ids:
                    invoice = self.env['account.invoice'].browse(dr_line.invoice_id.id)
                    ...
                    for cr_line in record.line_cr_ids:
                        if amount > 0.0 and cr_line.amount > 0.0 and not cr_line.move_line_id.reconciled:
                            ...
                            invoice.assign_outstanding_credit(cr_line.move_line_id.id)
                    # Linking with newly created payment
                    if payment_id and amount > 0:
                        payment_obj = self.env['account.payment'].browse(payment_id)
                        for move_line in payment_obj.move_line_ids:
                            if move_line.credit > 0.0 and not move_line.reconciled:
            ...
            # Processing customer receipts
                ...
                for cr_line in record.line_cr_ids:
                    invoice = self.env['account.invoice'].browse(cr_line.invoice_id.id)
                    amount = cr_line.amount
                    # Linking credit lines
                    for dr_line in record.line_dr_ids:
                        if amount > 0.0 and dr_line.amount > 0.0 and not dr_line.move_line_id.reconciled:
                            amount -= dr_line.amount
                            invoice.assign_outstanding_credit(dr_line.move_line_id.id)
                    # Linking with newly created payment
                    if payment_id and amount > 0:
                        payment_obj = self.env['account.payment'].browse(payment_id)
                        for move_line in payment_obj.move_line_ids:
                            if move_line.debit > 0.0 and not move_line.reconciled:
...
class ReceiptPaymentCredit(models.Model):
    ...
    original_amount = fields.Monetary(currency_field='currency_id', string='Base Currency Amount')
    amount_unreconciled_currency = fields.Monetary(currency_field='move_currency_id', string='Original Currency Open Balance')
    amount_unreconciled = fields.Monetary(currency_field='currency_id', string='Base Currency Open Balance')
...
#        if self.amount_unreconciled > 0 and float_round(self.amount, 2) > float_round(self.amount_unreconciled, 2):
#            raise ValidationError('Allocation amount must be less than or equal to Residual Amount.')
    ...
    def onchange_reconcile(self):
        ...
        if self.reconcile and ctx.get('reconcile'):
            ...
            self.amount = 0.0
        elif ctx.get('amount') and float_round(self.amount, 2) >= float_round(self.amount_unreconciled, 2):
            self.reconcile = True
        elif ctx.get('amount') and float_round(self.amount, 2) < float_round(self.amount_unreconciled, 2):
...
class AccountPayment(models.Model):
    ...
    def post(self):
        if self.env.context.get('type') == 'out_invoice':
            ...
            debit_vals = {}
            debit_vals['original_amount'] = 0
            debit_vals['original_amount_currency'] = 0
            debit_vals['amount_unreconciled_currency'] = 0
            debit_vals['amount_unreconciled'] = 0
            debit_vals['amount_residual'] = 0
            debit_vals['amount'] = 0
            for line in invoice.move_id.line_ids:
                if line.debit:
                    ...
                    if self.currency_id != invoice.company_id.currency_id:
                        debit_vals['original_amount'] += invoice.company_id.currency_id.with_context(date=self.payment_date).compute(abs(line.balance),
                                                                                               self.currency_id)
                    else:
                        debit_vals['original_amount'] += abs(line.balance)
                    debit_vals['original_amount_currency'] += line.amount_currency or abs(line.balance)
                    debit_vals['amount_unreconciled_currency'] += abs(line.amount_residual)
                    if self.currency_id != invoice.company_id.currency_id:
                        debit_vals['amount_unreconciled'] += invoice.company_id.currency_id.with_context(date=self.payment_date).compute(
                            abs(line.amount_residual), self.currency_id)
                    else:
                        debit_vals['amount_unreconciled'] += abs(line.amount_residual)
                    debit_vals['amount_residual'] += abs(line.amount_residual)
                    debit_vals['reconcile'] = True
                    debit_vals['amount'] += self.amount
            ...
            vals['currency_id'] = self.currency_id.id
            credit_vals = {}
            credit_vals['original_amount'] = 0
            credit_vals['original_amount_currency'] = 0
            credit_vals['amount_unreconciled_currency'] = 0
            credit_vals['amount_unreconciled'] = 0
            credit_vals['amount_residual'] = 0
            credit_vals['amount'] = 0
            for line in invoice.move_id.line_ids:
                if line.credit:
                    ...
                    if self.currency_id != invoice.company_id.currency_id:
                        credit_vals['original_amount'] += invoice.company_id.currency_id.with_context(date=self.payment_date).compute(abs(line.balance),
                                                                                                self.currency_id)
                    else:
                    ...
                    credit_vals['amount_unreconciled_currency'] += abs(line.amount_residual)
                    if self.currency_id != invoice.company_id.currency_id:
                        credit_vals['amount_unreconciled'] += invoice.company_id.currency_id.with_context(date=self.payment_date).compute(
                            abs(line.amount_residual),self.currency_id)
                    else:
                        credit_vals['amount_unreconciled'] += abs(line.amount_residual)
                    credit_vals['amount_residual'] += abs(line.amount_residual)
                    credit_vals['reconcile'] = True
                    credit_vals['amount'] += self.amount
