odoo.define('pos_restaurant_base.models', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        get_line_resume: function(line) {
            ...
            var attribute = line.get_attribute()
            var product_id = line.get_product().id;
            var product_name_wrapped = line.generate_wrapped_product_name();
            var line_id = line.id;
            return {qty: qty, note: note, product_id: product_id, product_name_wrapped: product_name_wrapped, line_id: line_id, attribute:attribute};
        },
        ...
        print_order_receipt: function(printer, changes) {
            ...
            if ( changes['new'].length > 0 || changes['cancelled'].length > 0){
                for (var i = changes['new'].length - 1; i >= 0; i--) {
                    console.log(JSON.stringify(changes['new'][i]))
                }
                q = q.then(function(){
