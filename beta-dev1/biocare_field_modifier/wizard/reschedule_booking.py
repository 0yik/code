    ...
    def action_reschedule(self):
        ...
        for obj in self:
            for wo_obj in self.env['stock.picking'].browse(
                ...
                is_available = True
                is_available = wo_obj.sudo().action_check_auto_allocate(
                ...
                # auto allocate logic
                    wo_obj.sudo().allocate_team(obj.start_date, obj.end_date, wo_obj, from_reschedule=True)
                reason = ''
                ...
                reason += 'Previous Scheduled End Date & Time: ' + datetime.datetime.strftime(
                    pytz.utc.localize(datetime.datetime.strptime(
                        ...
                        DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local),"%d/%m/%Y %H:%M:%S") + '\n'
                wo_obj.sudo().is_reschedule = True
                wo_obj.sudo().reschedule_start_date = obj.start_date
                wo_obj.sudo().scheduled_start= obj.start_date
                wo_obj.sudo().reschedule_end_date = obj.end_date
                wo_obj.sudo().scheduled_end = obj.end_date
                #wo_obj.reschedule_reason = obj.reschedule_reason
                wo_obj.sudo().reschedule_reason = reason

                # Search calendar event and reschedule it
                calendar_obj = calendar_event_pool.sudo().search(
                    [('name', '=', wo_obj.origin)])
                #if not found then we search on the basis of name of WO
                if not calendar_obj:
                    calendar_obj = calendar_obj.sudo().search(
                        [('name', '=', wo_obj.name)]
                    )
                if calendar_obj:
                    #calendar_obj.write({'active': False})
                    calendar_obj.sudo().write({'start': obj.start_date,
                                        'stop': obj.end_date,
                                        })
                bo_obj = bo_pool.sudo().search(
                    [('name', '=', wo_obj.origin)])
                if bo_obj:
                    reason = ''
                    reason += obj.reschedule_reason + '\n'
                    ...
                    bo_obj.sudo().write({
                        ...
                        'team': bo_obj.pick_id.team.id,
                    })
                    bo_obj.sudo()._onchange_team()
