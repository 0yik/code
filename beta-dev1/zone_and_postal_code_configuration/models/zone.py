...
class zones_postal_code(models.Model):
    _name = "zonepostal.code"

    name = fields.Char('Postal Code', size=2)
    ...
    def _check_cons_name(self):
        try:
            ...
            raise exceptions.ValidationError(_("Please enter number value for Allocated Postal Code.(Eg: 01)"))
        if len(str(self.name)) > 2 or len(str(self.name))< 2:
            raise exceptions.ValidationError(
                _('Please enter the first 2 digits of the Postal Code. Eg: 78'))

    _sql_constraints = [
        ('name_uniq', 'unique (name)', 'The postal code must be unique !')
    ]
    ...
    def name_get(self):
        ...
        if 'parent_postal_code_id' in self._context:
            ...
            return super(zones_postal_code,self).name_get()
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        if name and 'parent_postal_code_id' in self._context:
            if self._context.get('parent_postal_code_id', False) and self._context.get('parent_postal_code_id', False)[0] and self._context.get('parent_postal_code_id', False)[0][2]:
                ...
                return recs.with_context(ctx).name_get()
        return super(zones_postal_code,self).name_search(name, args=args, operator=operator, limit=limit)
...
class sequence_details(models.Model):
    ...
    postal_code_id = fields.Many2one('zonepostal.code', string="Postal Code")
...
class Zone(models.Model):
    ...
    postal_code_ids = fields.Many2many('zonepostal.code',
                                       'rel_zone_postal_code_final',
                                       'zone_id',
                                       'postal_code',
