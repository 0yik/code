...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def _create_invoice(self, order, so_line, amount):
        ...
        if self.advance_payment_method == 'percentage':
            ...
            if self.advance_payment_method == 'percentage':
                if self.rate:
                    # amount = (order.amount_total * self.rate) * (self.amount) / 100
                    amount = ((order.amount_untaxed - order.amount_discount) * self.rate) * (
        ...
        tax_ids = order.order_line.mapped('tax_id') and order.order_line.mapped('tax_id')[0]
