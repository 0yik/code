...
class AccountInvoice(models.Model):
    ...
    def _compute_amount(self):
        ...
        self.amount_total = ((self.amount_untaxed - self.amount_discount + self.amount_tax) - self.amount_commission)
    ...
    def invoice_line_move_line_get(self):
        ...
        res = []
        commission_bool = False
        discount_bool = False
        tax_bool = False
        total_amount_with_discount = 0.0
        is_calculate_discount_amount = False
        is_discount_calculated = False
        is_amount_calculated = False
        if self.amount_discount and self.discount_type == 'amount':
            for line in self.invoice_line_ids:
                line_price = line.price_subtotal
                if line_price < self.amount_discount:
                    is_calculate_discount_amount = True
                else:
                    is_calculate_discount_amount = False
                    break
        for line in self.invoice_line_ids:
            length_invoice = len(self.invoice_line_ids)
            if line.quantity==0:
                continue
            tax_ids = []
            ...
            amount_tax = 0
            amount_commission = 0
            is_discount_amount = 0.0
            if self.amount_discount:
                if self.discount_type == 'amount' and price > self.amount_discount and discount_bool == False:
                    amount_discount = self.discount_rate
                    discount_bool = True
                if self.discount_type == 'percent':
                    amount_discount = (line.price_subtotal * self.discount_rate) / 100
            if self.amount_tax:
                if self.discount_type == 'amount' and price > self.tax_rate and tax_bool == False:
                    amount_tax = self.tax_rate
                    tax_bool = True
                if self.discount_type == 'percent':
                    amount_tax = (line.price_subtotal * self.tax_rate ) / 100
            if self.amount_commission:
                if self.commission_type == 'amount' and price > self.commission_rate and commission_bool == False:
                    amount_commission = self.commission_rate
                    commission_bool = True
                if self.commission_type == 'percent':
                    amount_commission = (((line.price_subtotal - amount_discount ) * self.commission_rate ) / 100)
            if is_calculate_discount_amount:
                is_discount_amount = True
                if not is_amount_calculated:
                    amount_discount = self.discount_rate
                    price = abs(price) - float_round(amount_discount, precision_digits=precision) + amount_tax - amount_commission
                    is_amount_calculated = True
                elif total_amount_with_discount < 0.0:
                    price = total_amount_with_discount + price + amount_tax - amount_commission
                else:
                    price = price - float_round(amount_discount, precision_digits=precision) + amount_tax - amount_commission
                total_amount_with_discount = price
                if total_amount_with_discount < 0.0:
                    price = 0.0
                else:
                    price = price
            elif not is_discount_calculated:
                is_discount_amount = True
                price = price - float_round(amount_discount, precision_digits=precision) + amount_tax - amount_commission
                if price > 0.0:
                    price = price
                    is_discount_calculated = True
            if not is_discount_amount:
                price = price - float_round(amount_discount, precision_digits=precision) + amount_tax - amount_commission
            move_line_dict = {
                'invl_id': line.id,
                'type': 'src',
    ...
    @api.multi
    def amount_calculation_residual(self):
        self.amount_untaxed = sum(line.price_subtotal for line in self.invoice_line_ids)
        self.amount_discount = self.amount_tax = self.amount_commission = 0.0
        if self.discount_type == 'amount':
            self.amount_discount = self.discount_rate
    ...
    def action_invoice_open(self):
        ...
        # called = to_open_invoices.action_create_commissin_move()
    ...
    def action_create_commissin_move(self):
        if self.journal_id:
            ...
            referral_fee_account_obj = self.env['ir.model.data'].get_object('solum_invoice', 'luminiare_referal_fee_account')
            #other_payable_account_obj = self.env['ir.model.data'].get_object('l10n_sg', 'account_account_778')
            other_payable_account_obj = self.env['account.account'].search([('code','=','202002')])
