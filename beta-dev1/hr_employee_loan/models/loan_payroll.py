...
#             period_id = period_pool.find(loan.date_applied)[0]
            timenow = time.strftime('%Y-%m-%d')
            address_id = loan.employee_id.address_home_id
            partner_id = address_id and address_id.id

            if not partner_id:
                raise Warning(_('Please configure Home Address On Employee.')) 
            
            move = {
                'narration': loan.name if loan.name else '',
                'date': loan.date_disb or datetime.now().date(),
                'ref': loan.name if loan.name else '',
                'journal_id': loan.journal_id.id if loan.journal_id and loan.journal_id.id else False,
#                 'period_id': period_id.id,
            ...
            debit_line = (0, 0, {
                    ...
                    'account_id': debit_account_id if debit_account_id else False,
                    'journal_id': loan.journal_id.id if loan.journal_id and loan.journal_id.id else False,
#                     'period_id': period_id.id,
                    'debit': loan.principal_amount if loan.principal_amount else 0.0,
                    'credit': 0.0,
                })
            credit_line = (0, 0, {
                    'name': _('Loan of %s') % (loan.employee_id.name),
                    'date': loan.date_disb or datetime.now().date(),
                    'partner_id': partner_id if partner_id else False,
                    'account_id': credit_account_id if credit_account_id else False,
                    'journal_id':  loan.journal_id.id if loan.journal_id and loan.journal_id.id else False,
#                     'period_id': period_id.id,
                    'debit': 0.0,
                    'credit':loan.principal_amount if loan.principal_amount else 0.0,
                })
            move.update({'line_ids': [debit_line, credit_line]})
            move_id = move_pool.create(move)
